...
parse(obj,json,schema,ref) ; obj() branch key (optional) - adds dimensions to ref
 ...
 I (":,[{"[$EXTRACT(json,$LENGTH(json))) D error("Unexpected delimiter",$LENGTH(json))
 ;
 ; tokenize location of json delimiters :,[]{}
 N tjson S tjson=$TRANSLATE(json,":,[]{}",$CHAR(1,1,1,1,1,1))
 ;
 ...
 ;
 F  S y=$FIND(tjson,$char(1),y) D  I (y>$LENGTH(json)) Q
 .	;
 .	S ye=y-2
 ...
 .	I (qp>0&(qp'>(y-1))) D  I q S y=$S(qp:qp,1:$LENGTH(json)+1) Q
 ..		S q=($EXTRACT(json,qp-2)'="\")
 ..		F  S qp=$FIND(json,"""",qp) Q:(qp=0!(qp-1>ye))  I ($EXTRACT(json,qp-2)'="\") S q='q
 ..		Q
 .	;
 .	S del=$EXTRACT(json,y-1) ; get json delimiter
 .	;
 .	; Move ye before whitespace at end and yt past whitespace at beginning
 .	F  Q:'(charStrip[$EXTRACT(json,ye))!(ye<yt)  S ye=ye-1
 .	F  Q:'(charStrip[$EXTRACT(json,yt))!(yt>ye)  S yt=yt+1
 .	;
 .	S val=$EXTRACT(json,yt,ye)
 .	S yt=y
 .	;
 .	I (","[del) D  Q
 ..		;
 ..		I $EXTRACT(json,y)="," D error("Value expected before ','",y) Q
 ..		I '(val="") D setVal
 ..		;
 ..		I ($EXTRACT(stack,1)="]") S pk(level)=pk(level)+1
 ..		E  S pk(level)=""
 ..		Q
 .	;
 .	; Tag syntax
 ...
 .	I ("}]"[del) D  Q
 ..		;
 ..		I '(del=$EXTRACT(stack,1)) D error($S($EXTRACT(stack,1)="}":"Object",1:"Array")_" terminator expected",y-1)
 ..		I '(val="") D setVal
 ..		;
 ..		I level S level=level-1
 ..		S stack=$EXTRACT(stack,2,1048575)
 ..		Q
 .	Q
 ;
 I q D error("Closing string delimiter "" expected",yt)
 I '(stack="") D error("Terminator expected '"_stack_"'",$LENGTH(json)+1)
 ;
 Q
 ;
...
stringify(obj,schema,ref) ; Primary key reference (to grab a branch only)
 ...
 ..		;
 ..		S ret=ret_$EXTRACT(stack,1,savLevel-hwaLevel)
 ..		S stack=$EXTRACT(stack,savLevel-hwaLevel+1,1048575)
 ..		Q
 .	;
 ...
 ..		I (tag?1N.N) D  Q:ok(i)
 ...			I (tag=1) D  Q
 ....				I (ok(i)="") S ret=ret_"[" S stack="]"_stack S arptr(i)=$LENGTH(ret) S ok(i)=1
 ....				Q
 ...			;
 ...
 ..		;
 ..		; tag: value
 ..		I '("{[,:"[$EXTRACT(ret,$LENGTH(ret))) S ret=ret_","
...
indexToTag(ptr,index) ; Convert an array to tags (forced when M array is sparse)
 ;
 N x S x=$EXTRACT(ret,ptr+1,1048575)
 S ret=$EXTRACT(ret,1,ptr-1)_"{"
 S stack="}"_$EXTRACT(stack,2,1048575)
 ;
 I index=1 S ret=ret_"""1"":"_x_"," Q
 ;
 N zx S zx=$TRANSLATE(x,"""[]{},",$C(1,1,1,1,1,1))_$char(1)
 N i S i=1 N y S y=1 N yb S yb=1 N ye
 N c N s S s=""
 ;
 F  S y=$F(zx,$char(1),y) Q:y=0  D  I y=0 Q
 .	S ye=y-2
 .	I ($EXTRACT(x,ye)=" ") F ye=ye-1:-1 Q:'($EXTRACT(x,ye)=" ")
 .	I y>$L(zx) S ret=ret_""""_i_""":"_$EXTRACT(x,yb,ye)_"," Q
 .	S c=$EXTRACT(x,y-1)
 .	I c="""" F  S y=$FIND(x,"""",y) Q:'y!'($EXTRACT(x,y-2)="\")
 .	I $EXTRACT(x,y)=" " F y=y+1:1 Q:'($EXTRACT(x,y)=" ")
 .	I c="," I (s="") S ret=ret_""""_i_""":"_$EXTRACT(x,yb,ye)_"," S i=i+1 S yb=y Q
 .	I "[{"[c S s=c_s Q
...
pop(obj,json,levels,schema,ref) ;
 ...
 .	S qp=$FIND(json,"""",yt)
 .	; if quotes exist before delimiter, count unescaped quotes, if 'odd' number then quit
 .	I (qp>0&(qp'>(y-1))) D  I q S y=$S(qp:qp,1:$LENGTH(json)) Q
 ..		S q=($EXTRACT(json,qp-2)'="\")
 ..		F  S qp=$FIND(json,"""",qp) Q:(qp=0!(qp-1>ye))  I ($EXTRACT(json,qp-2)'="\") S q='q
 ..		Q
 .	;
...
getClose ; Get the closing delimiter for pop
 ...
 N del2
 ;
 F  S y=$FIND(tjson,$char(1),y) D  I (y>$LENGTH(json)!(level=levels)) Q
 .	;
 .	S ye=y-2
 .	;
 .	; If json delimiter is protected inside quotes, skip it
 ...
 ..		Q
 .	;
 .	S del2=$EXTRACT(json,y-1)
 .	I ("{["[del2) S level=level+1
 .	E  I ("]}"[del2) S level=level-1
 .	Q
 ;
 S val=del_$EXTRACT(json,yt,y-1)
 S yt=y
 ;
 D setVal
 Q
...
getType() ; Check if tag has schema associated with type Date, Time, Bool
 ...
 I (z=tag)!($EXTRACT(z,$LENGTH(z))="*")&($EXTRACT(tag,1,$LENGTH(z)-1)=$EXTRACT(z,1,$LENGTH(z)-1)) Q schema(z)
 ;
 ; What the heck, could get lucky and match the last key in *xxx patterns
 I $EXTRACT(z,1)="*" I ($EXTRACT(tag,$LENGTH(tag)-$LENGTH(z)+2,1048575)=$EXTRACT(z,2,1048575)) Q schema(z)
 ;
 ; Continue search for beginsWith pattern: xxx*
 ; Go back in schema array until keys is out of range or 'xxx*' found
 I ($EXTRACT(tag,1,$LENGTH(z))=z) F  S z=$order(schema(z),-1) Q:(z="")!($EXTRACT(z,$LENGTH(z))="*")!'($EXTRACT(tag,1,$LENGTH(z))=z)
 I   I $EXTRACT(z,$LENGTH(z))="*" I $EXTRACT(tag,1,$LENGTH(z)-1)=$EXTRACT(z,1,$LENGTH(z)-1) Q schema(z)
 ;
 I (z="") Q ""
 ;
 ; Now check all of the endsWith pattern: *xxx
 S z="*"
 F  S z=$order(schema(z)) Q:'($EXTRACT(z,1)="*")!($EXTRACT(tag,$LENGTH(tag)-$LENGTH(z)+2,1048575)=$EXTRACT(z,2,1048575))
 I $EXTRACT(z,1)="*" Q schema(z)
 ;
 Q ""
 ;
escape(str) ; Find and replace JSON escape character in M string
 ...
 N cstr S cstr=$TRANSLATE(str,$CHAR(34,47,92,8,9,10,12,13),"^^^^^^^^")
 ;
 ; Then compare strings, if different there are escape characters
 I (cstr'=str) D
 .	N y S y=0 N c S c=0
 .	F  S y=$FIND(cstr,"^",y) Q:y=0  I ($EXTRACT(str,y+c-1)'="^") D
 ..		S str=$EXTRACT(str,1,y+c-2)_"\"_$E("""/\btnfr",$F($CHAR(34,47,92,8,9,10,12,13),$EXTRACT(str,y+c-1))-1)_$EXTRACT(str,y+c,1048575)
 ..		S c=c+1
 ..		Q
 .	Q
 ;
...
jstrToMstr(str,tag) ; Convert from a JSON string to M string
 ...
 .	;
 .	I y F  Q:'y  D
 ..		S str=$EXTRACT(str,1,y-2)_$TRANSLATE($EXTRACT(str,y),"bfnrt",$CHAR(8,12,10,13,9))_$EXTRACT(str,y+1,1048575)
 ..		S y=$FIND(str,"\",y)
 ..		Q
 .	Q
 ;
 I $LENGTH(str)<2 Q str
 ;
 ; Remove quote string delimiters
 I $EXTRACT(str,1)=""""&($EXTRACT(str,$LENGTH(str))="""") D  Q str
 .	S str=$EXTRACT(str,2,$LENGTH(str)-1)
 .	;check for Date or Time schema, convert if match
 .	I ($EXTRACT(str,3)=":"!($EXTRACT(str,5)="-")) D
 ..		S dt=$$getType()
 ..		I dt="Date" S str=$$getJulien(str) Q
 ..		I dt="Time" S str=(($EXTRACT(str,1,2)*3600)+($EXTRACT(str,4,5)*60)+$EXTRACT(str,7,8)) Q
 ..		Q
 .	Q
 ;
 Q str
...
jtagToMtag(str) ; Convert a Json tag to an M tag, no need to look for keywords or schema (so faster processing)
 ...
 .	Q
 ;
 I $EXTRACT(str,1)="""" S str=$EXTRACT(str,2,$LENGTH(str)-1)
 Q
 ;
getJulien(str) ; Return Julien date from ISO 8601 String YYYY-MM-DDT24:60:SS"
 ;
 N year S year=$EXTRACT(str,1,4)
 N month S month=$EXTRACT(str,6,7)
 N jdate S jdate=$EXTRACT(str,9,10)
 ;
 ...
 ;
 ; TimeStamp
 I ($EXTRACT(str,11)="T") S jdate=jdate_","_(($EXTRACT(str,12,13)*3600)+($EXTRACT(str,15,16)*60)+$EXTRACT(str,18,19))
 Q jdate
 ;
