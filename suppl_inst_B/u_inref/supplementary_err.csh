...
echo "# Expect rcvr to exit with GTM-E-UPDSYNCINSTFILE when no value is specified to -updateresync"
...
echo "# Expect the rcvr to exit with GTM-E-REPL2OLD if it attempts to connect to pre V5.5-000 source"
...
echo "# Expect the src to exit with GTM-E-REPL2OLD if it attempts to connect to pre V5.5-000 receiver"
...
# SECNOTSUPPLEMENTARY
# %GTM-E-SECNOTSUPPLEMENTARY, INSTANCE4 is a Supplementary Instance and so cannot act as a source to non-Supplementary Instance INSTANCE1
echo "# Expect src of INST4 which is supplementary to exit with GTM-E-SECNOTSUPPLEMENTARY when it tries to connect to rcvr of INST1, which is non-supplementary"
...
# SUPRCVRNEEDSSUPSRC
# %GTM-E-SUPRCVRNEEDSSUPSRC, Instance INSTANCE5 is not configured to perform local updates so it cannot act as a receiver for non-Supplementary Instance INSTANCE1
echo "# Expect rcvr of INST5 which is supplementary, but no -updok server to exit with GTM-E-SUPRCVRNEEDSSUPSRC when it tries to connect to src of INST1"
...
# NOSUPPLSUPPL
# %GTM-E-NOSUPPLSUPPL, Instance INSTANCE4 is configured to perform local updates so it cannot receive from Supplementary Instance INSTANCE6
echo "# Expect RCVR of INST4 which is a supplementary instance to error out with GTM-E-NOSUPPLSUPP, when it connects to INST6 which is also a supplementary instance"
...
echo "# Expect GTM-E-REPLINSTSECLEN if the length of -instsecondary is >15 bytes"
...
# INSROLECHANGE
echo "# Expect GTM-E-INSROLECHANGE below"
