...
foreach instance (INST1 INST2)
	...
	$MSR RUN $instance 'set msr_dont_trace; $gtm_exe/mumps -run jnlflush^ydb722; $gtm_tst/com/jnlextall.csh mumps >& jnlextall.out; $grep -E "SET|ZTRIG|LGTRIG|NULL|KILL|ZTWORM|TCOM" mumps.mjf' | grep -v '#t' | cut -b 24- > $mjffile
	switch ($testname)
	case "test1":
	case "test3":
	case "test4":
