...
#################################################################
#
# crash: 	If database/journaling is crashed, backward recovery should not continue. (evo_tn > curr_tn)
...
BEGIN "Fill the small.dat with data"
...
BEGIN "small.dat will be used for recovery. So get jnl_eovtn and curr_tn from database file header"
...
BEGIN "Get the eov from journal file header"
        $MUPIP journal -show=header -backward big.mjl >&! jnlheader.txt
...
#CASE (crash and jfh->eov_tn > db->eov_tn)
mv small.dat big.dat      # tn in database is made smaller.
mv big.mjl_bak small.mjl  # Rename journal file same as that it is present in database.
setenv gtmgbldir big.gld
...
BEGIN "Recover should fail: jfh->eov_tn > db->curr_tn"
