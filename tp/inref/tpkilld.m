TPKILLD	; ; ; test index block deletion in the middle
	;
; Create a index block with several chain recs, then delete some 
; records from the middle of the block so that the chain
; does not extend beyond the deleted region.
; 
; This case differs from tpkillc in that there is only one chain
; record, so we end up with no chain records in the block when
; we complete this deletion.
; this has worked and should continue to work
;
;                    xxxxxxxxxxxxx
;   -----------------------------------------------
;   | |      |      |      |      |      |        |
;   | |      |      |      |      |      |        |
;   |b| ^A(1)| ^A(1,| ^A(1,| ^A(1,| *-key|        |
;   |h|      |    1)|    1,|    1,|      |        |
;   | |      |      |    1)|    1,|      |        |
;   | |      |      |      |    1)|      |        |
;   | |      |      |      |      |      |        |
