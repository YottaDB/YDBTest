...
echo "# Test that compiling boolean expressions involving NOT operator (') and side effects does not spin loop if ydb_boolean=1"
cp $gtm_tst/$tst/inref/ydb553*.m .

foreach bool ( 0 1 )
	source $gtm_tst/com/set_ydb_env_var_random.csh ydb_boolean gtm_boolean $bool
	echo " --> Test with ydb_boolean env var set to $bool"
	foreach file (ydb553*.m)
		set basename = $file:r
