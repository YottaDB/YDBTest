...
; for 8 seconds in a loop keeping track of number of computations done.
; Expected behavior: current version has higher number of computations than the previous version.
compdectohex
	set ver=$piece($zcmdline," ",1)
	set digits=$piece($zcmdline," ",2)

	; Seed values hardcoded as this is a performance test
	set:16=digits seed=21474836
	set:20=digits seed=1844674407
	set:20=digits seed2=3709551615

	write "Running performance benchmark for "_$select("p"=ver:"previous",1:"current")_" %DH implementation with "_digits_"-digit values",!
	for i=1:1:10000  do
	. set d=$random(seed)_$random($select(16=digits:seed,1:seed2))
	. set:"c"=ver k=$$FUNC^%DH(d,) ; "c"urrent version
	. set:"p"=ver k=$$FUNCPREVDECTOHEX(d,digits) ; "p"revious version
...
comphextodec
	...
	; Seed values hardcoded as this is a performance test
	set:14=digits seed=2147483
	set:16=digits seed=21474836

	write "Running performance benchmark for "_$select("p"=ver:"previous",1:"current")_" %HD implementation with "_digits_"-digit values",!
	for i=1:1:10000  do
	. set d=$random(seed)_$random(seed)
	. set:"c"=ver k=$$FUNC^%HD(i) ; "c"urrent version
	. set:"p"=ver k=$$FUNCPREVHEXTODEC(i) ; "p"revious version
