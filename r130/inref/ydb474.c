...
int main()
{
	...
	status = ydb_decode_s(&decode_to, 0, subscr31, 31, json, json_string);
	if (YDB_OK != status)
	{
		ydb_zstatus(errbuf, ERRBUF_SIZE);
		printf("ydb_decode_s() [1]: %s\n", errbuf);
		fflush(stdout);
		return YDB_OK;
	}
	printf("Decode valid JSON array\n"); fflush(stdout);
	strcpy(decode_to.buf_addr, "str2");
	status = ydb_decode_s(&decode_to, 0, subscr31, 31, json, json_string2);
	if (YDB_OK != status)
	{
		ydb_zstatus(errbuf, ERRBUF_SIZE);
		printf("ydb_decode_s() [1]: %s\n", errbuf);
	...
	status = ydb_decode_s(&decode_to, 0, subscr31, 31, json, json_string3);
	if (YDB_OK != status)
	{
		ydb_zstatus(errbuf, ERRBUF_SIZE);
		printf("ydb_decode_s() [1]: %s\n", errbuf);
	...
	status = ydb_decode_s(&decode_to, 0, subscr31, 31, json, json_string4);
	if (YDB_OK != status)
	{
		ydb_zstatus(errbuf, ERRBUF_SIZE);
		printf("ydb_decode_s() [1]: %s\n", errbuf);
	...
	status = ydb_decode_s(&decode_to, 0, subscr31, 31, json, value);
	if (YDB_OK != status)
	{
		ydb_zstatus(errbuf, ERRBUF_SIZE);
		printf("ydb_decode_s() [2]: %s\n", errbuf);
		fflush(stdout);
	}
	printf("Decode valid JSON that would need too many subscripts\n"); fflush(stdout);
	strcpy(decode_to.buf_addr, "str5");
	decode_to.len_used = 4;
	status = ydb_decode_s(&decode_to, 0, subscr31, 31, json, json_string_too_many_subscripts);
	if (YDB_OK != status)
	{
		ydb_zstatus(errbuf, ERRBUF_SIZE);
		printf("ydb_decode_s() [2]: %s\n", errbuf);
	...
	decode_to.len_used = 4;
	status = ydb_decode_s(&decode_to, 0, subscr31, 31, json, json_string6);
	if (YDB_OK != status)
	{
		ydb_zstatus(errbuf, ERRBUF_SIZE);
		printf("ydb_decode_s() [2]: %s\n", errbuf);
	...
	printf("### Test encodes in ydb_encode_s() using the decoded JSON objects ###\n"); fflush(stdout);
	for (i = 0; i < 31; i++)
	{
	...
	printf("\n");
	status = ydb_encode_s(&input, cur_num_subscripts, subscripts_array, json, &encoded_data);
	if (YDB_OK != status)
	{
		ydb_zstatus(errbuf, ERRBUF_SIZE);
		printf("ydb_encode_s() [2]: %s\n", errbuf);
		fflush(stdout);
		return YDB_OK;
	}
	else
