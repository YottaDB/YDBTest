;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;
;								;
;	Copyright 2007, 2015 Fidelity Information Services, Inc	;
;								;
...
;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;
;
; C9G04-002779 - Socket read is not Mupip interrupt-able
;
; Test flow:
...
;
	...
	Set $Zint="Set ^zintmaincnt=^zintmaincnt+1"
        Set $Ztrap="Set $Ztrap="""" Set ^done=1 Set ^done(""tsockzintr"")=$Zpos Use $P Zshow ""*"" Halt"

	; Fork off peer
        Write "Spawning peer job",!
        If unix Job @("commpeer^tsockzintr():(output=""commpeer.mjo"":error=""commpeer.mje"")")
        Else    Job @("commpeer^tsockzintr():(nodetached:startup=""startup.com"":output=""commpeer.mjo"":error=""commpeer.mje"")")
	...
	Write !,"Peer job active",!
        ...
        If unix Job @("intrdrv^tsockzintr():(output=""intrdrv.mjo"":error=""intrdrv.mje"")")
        Else    Job @("intrdrv^tsockzintr():(nodetached:startup=""startup.com"":output=""intrdrv.mjo"":error=""intrdrv.mje"")")
	Write "."
	...
	. . Set ^error($H,"client")="TSOCKZINTR-E-FAILED to enable TLS: "_dev_" at "_$Zpos
	. . zshow "D":^error(errtime,"client","zshowd")
	. . Use $P Write "TLS client enable failed: "_dev,!
	. . Set ^done=1,^done("TLS client enable failed")=$Zpos
	. . Halt
...
commpeer  ;;;	    The peer process
	...
	. Set ^error($H,"server")="TSOCKZINTR-E-FAILED Piece count in random string was "_lenrands_" instead of "_^rancnt
	. Use $P
	. ZShow "*"
	...
	. Read rcvstr:60
	. If '$T Do
        . . Set ^error($H,"server")="TSOCKZINTR-E-FAILED Read "_i_" timed out - Read start: "_readstart_"  Read end: "_$H
	...
	. If expectlen'=recvlen Do
        . . Set ^error($H,"server")="TSOCKZINTR-E-FAILED Read "_i_" is not the expected length - expected "_expectlen_" received "_recvlen
	...
	. If expectstr'=rcvstr Do
        . . Set ^error($H,"server")="TSOCKZINTR-E-FAILED Read "_i_" expected and received text do not match"
...
peersetup	;;;
	Set portno=^config("portno"),delim=^config("delim")
        ...
        Open tcpdev:@openarg:timeout:"SOCKET"
        Else  Set ^error($H,"server")="TSOCKZINTR-E-FAILED to open socket at "_$Zpos  Use $P Write "open failed : $zstatus=",$Zstatus,! Set ^done=1,^done("Peer open failed")=$Zpos Halt
        Use tcpdev
        Write /listen(1)
	Set ^peerpid=$j
	Set ^peeractive=1		; signal main we are running
        Write /wait(timeout)
	If ""=$Key do
	. Set ^error($H,"server")="TSOCKZINTR-E-FAILED to establish connection at "_$Zpos
	...
	. If (test=0)!(+$Device'=0)!(tlsset'="TLS") Do
	. . New dev,errtime Set dev=$Device,errtime=$H
	. . Set ^error($H,"server")="TSOCKZINTR-E-FAILED to enable TLS: "_dev_" at "_$Zpos
	. . zshow "D":^error(errtime,"server","zshowd")
	. . Use $P Write "TLS server enable failed: "_dev,!
	. . Set ^done=1,^done("TLS server enable failed")=$Zpos
	. . Halt
        Quit
