...
echo "-----------------------------------------------------------------------------------------------------"
echo "# Test that ydb_eintr_handler() helps terminate SimpleAPI application if a SIGINT/SIGTERM is received"
echo "-----------------------------------------------------------------------------------------------------"
set file = "ydb560"

echo "# Create ydb560loop.c from ydb560halt.c by removing ydb_eintr_handler() call"
cp $gtm_tst/$tst/inref/ydb560halt.c .
sed 's/ydb_eintr_handler();//;' ydb560halt.c > ydb560loop.c
...
foreach file ("ydb560loop" "ydb560halt")
	...
	foreach sig (15 2)
		echo "# Run SimpleAPI application [`pwd`/$file] in background with signal number [SIG-$sig]"
		(`pwd`/$file < /dev/null > $file.$sig.out & ; echo $! >&! $file.$sig.pid) >& $file.$sig.err

		echo "# Sleep 1 second to give process some time to start"
		sleep 1

		echo "# Sending : [kill -$sig] to backgrounded SimpleAPI application"
		set bgpid = `cat $file.$sig.pid`
		kill -$sig $bgpid

		echo "# Waiting for backgrounded SimpleAPI application to terminate"
		if ("ydb560loop" == "$file") then
			echo "# No ydb_eintr_handler() used : Process is not expected to terminate"
			set sleeptime = 1
		else
			echo "# ydb_eintr_handler() is used : Process is expected to terminate"
			set sleeptime = 60
		endif
