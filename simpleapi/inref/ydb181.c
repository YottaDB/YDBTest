...
/* Test PARAMINVALID and INVVARNAME errors from an invalid varname parameter in various Simple API calls */
int main()
{
	...
	printf("# Also test that PARAMINVALID error correctly identified ydb_set_s() as the caller function\n");
	...
	printf("# Test ydb_lock_s() with multiple lock names one of which is invalid identifies the invalid name\n");
	YDB_LITERAL_TO_BUFFER("lockA", &basevarname1);
	YDB_LITERAL_TO_BUFFER("1lockB", &basevarname2);
	status = ydb_lock_s(LOCK_TIMEOUT, 2, &basevarname1, 0, NULL, &basevarname2, 0, NULL);
	if (YDB_OK != status)
	{
		ydb_zstatus(errbuf, ERRBUF_SIZE);
		printf("%s\n", errbuf);
