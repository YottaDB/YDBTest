...
int main()
{
	...
	printf("### Test simple ydb_subscript_next_s() of Local Variables ###\n"); fflush(stdout);
	...
	status = ydb_subscript_next_s(&nextvar, 0, NULL, &ret_value);
	...
	ret_test = ret_value.len_used;
	memcpy(rettestbuff, ret_value.buf_addr, ret_value.len_used);
	status = ydb_subscript_next_s(&nextvar, 1, subscr, &ret_value);
	if (YDB_ERR_NODEEND != status)
	{
		ydb_zstatus(errbuf, ERRBUF_SIZE);
		printf("ydb_subscript_next_s() did not return YDB_ERR_NODEEND: %s\n", errbuf);
	...
	ret_value.buf_addr[ret_value.len_used] = '\0';
	printf("ydb_subscript_next_s() returned [%s]\n", ret_value.buf_addr);
	printf("# Get next local variable of local variable with 2 subscripts\n"); fflush(stdout);
	ret_test = ret_value.len_used;
	memcpy(rettestbuff, ret_value.buf_addr, ret_value.len_used);
	status = ydb_subscript_next_s(&nextvar, 2, subscr, &ret_value);
	if (YDB_ERR_NODEEND != status)
	{
		ydb_zstatus(errbuf, ERRBUF_SIZE);
		printf("ydb_subscript_next_s() did not return YDB_ERR_NODEEND: %s\n", errbuf);
