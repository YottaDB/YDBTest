...
int main(){
	...
	printf("Calling ydb_get_s() to get the process in SimpleAPI mode\n");
	...
	if(status == YDB_OK){
		printf("ydb_thread_is_main() correctly returns YDB_OK in main thread\n");
	} else {
		printf("ydb_thread_is_main() is not returning YDB_OK in main thread\n");
	}
