...
int main(int argc, char** argv)
{
	...
	printf("### Test error scenarios in ydb_node_previous_s() of Global Variables ###\n\n"); fflush(stdout);
	...
	printf("# Attempting ydb_node_previous_s() of bad basevar (%% in middle of name): %s\n", BADBASEVAR1);
	fflush(stdout);
	status = ydb_node_previous_s(&badvar1, 0, NULL, &subs, &ret_value);
	if (YDB_OK != status)
	{
		ydb_zstatus(errbuf, ERRBUF_SIZE);
		printf("ydb_node_previous_s() [%d]: %s\n", __LINE__, errbuf);
		fflush(stdout);
	}
	printf("# Attempting ydb_node_previous_s() of bad basevar (first letter in name is digit): %s\n", BADBASEVAR3);
	status = ydb_node_previous_s(&badvar2, 0, NULL, &subs, &ret_value);
	if (YDB_OK != status)
	{
		ydb_zstatus(errbuf, ERRBUF_SIZE);
		printf("ydb_node_previous_s() [%d]: %s\n", __LINE__, errbuf);
	...
	printf("# Attempting ydb_node_previous_s() of bad basevar (> 31 characters): %s\n", BADBASEVAR2);
	status = ydb_node_previous_s(&badvar3, 0, NULL, &subs, &ret_value);
	if (YDB_OK != status)
	{
		ydb_zstatus(errbuf, ERRBUF_SIZE);
		printf("ydb_node_previous_s() [%d]: %s\n", __LINE__, errbuf);
	...
	printf("# Attempting ydb_node_previous_s() of basevar with 32 subscripts\n");
	for (i = 0; i < 32; i++)
		YDB_LITERAL_TO_BUFFER(SUBSCR32, &subscr32[i]);
	status = ydb_node_previous_s(&basevar, 32, subscr32, &subs, &ret_value);
	if (YDB_OK != status)
	{
		ydb_zstatus(errbuf, ERRBUF_SIZE);
		printf("ydb_node_previous_s() [%d]: %s\n", __LINE__, errbuf);
	...
	printf("# Attempting ydb_node_previous_s() of basevar with -1 subscripts\n");
	status = ydb_node_previous_s(&basevar, -1, NULL, &subs, &ret_value);
	if (YDB_OK != status)
	{
		ydb_zstatus(errbuf, ERRBUF_SIZE);
		printf("ydb_node_previous_s() [%d]: %s\n", __LINE__, errbuf);
	...
	printf("# Attempting ydb_node_previous_s() with *ret_subs_used=0 : Expect INSUFFSUBS error\n"); fflush(stdout);
	...
	status = ydb_set_s(&basevar,1,&node2,&value);
	if (YDB_OK != status)
	{
		ydb_zstatus(errbuf, ERRBUF_SIZE);
		printf("ydb_set_s() [%d]: %s\n", __LINE__, errbuf);
	...
	subs = 0;
	status = ydb_node_previous_s(&basevar,1,&node2,&subs,&ret_value);
	if (YDB_OK != status)
	{
		ydb_zstatus(errbuf, ERRBUF_SIZE);
		printf("ydb_node_previous_s() [%d]: %s\n", __LINE__, errbuf);
	...
	printf("\n# Test of INVSTRLEN\n"); fflush(stdout);
	printf("# Attempting ydb_node_previous_s() with ret_sub_array[0]:len_alloc set to 0 whereas return value is of length 1 : Expect INVSTRLEN error\n"); fflush(stdout);
	status = ydb_node_previous_s(&basevar,1,&node2,&subs,&bad_ret);
	if (YDB_OK != status)
	{
		ydb_zstatus(errbuf, ERRBUF_SIZE);
		printf("ydb_node_previous_s() [%d]: %s\n", __LINE__, errbuf);
	...
	printf("# Attempting ydb_node_previous_s() with ret_subarray->buf_addr = NULL : Expect PARAMINVALID error\n");
	...
	printf("# Attempting ydb_node_previous_s() with *ret_subarray = NULL : Expect PARAMINVALID error\n"); fflush(stdout);
	status = ydb_node_previous_s(&basevar, 1, &node2, &subs, NULL);
	if (YDB_OK != status)
	{
		ydb_zstatus(errbuf, ERRBUF_SIZE);
		printf("ydb_node_previous_s() [%d]: %s\n", __LINE__, errbuf);
	...
	printf("# Attempting ydb_node_previous_s() with ret_subs_used = NULL : Expect PARAMINVALID error\n"); fflush(stdout);
	status = ydb_node_previous_s(&basevar, 0, NULL, NULL, &ret_value);
	if (YDB_OK != status)
	{
		ydb_zstatus(errbuf, ERRBUF_SIZE);
		printf("ydb_node_previous_s() [%d]: %s\n", __LINE__, errbuf);
	...
	printf("# Attempting ydb_node_previous_s() with the previous node subscript = \"\" and ret_value->buf_addr = NULL : Expect NO PARAMINVALID error\n");
	fflush(stdout);
	subs = 1;
	printf("Set %s variable to null string: ", argv[2]); fflush(stdout);
	status = ydb_set_s(&nullvar, 0, &emptyval, &value);
	if (YDB_OK != status)
	{
		ydb_zstatus(errbuf, ERRBUF_SIZE);
		printf("ydb_set_s() [%d]: %s\n", __LINE__, errbuf);
	...
	status = ydb_node_previous_s(&nullvar, 1, &node1, &subs, &ret_value);
	if (YDB_OK != status)
	{
		ydb_zstatus(errbuf, ERRBUF_SIZE);
		printf("ydb_node_previous_s() [%d]: %s\n", __LINE__, errbuf);
	...
	{
		printf("ydb_node_previous_s() returns %s(\"\")\n", nullvar.buf_addr);
		fflush(stdout);
	}
	subs = 1;
	printf("\n# Test of SUBSARRAYNULL error\n"); fflush(stdout);
	printf("# Attempting ydb_node_previous_s() with *subsarray = NULL : Expect SUBSARRAYNULL error\n"); fflush(stdout);
	status = ydb_node_previous_s(&basevar, 1, NULL, &subs, &ret_value);
	if (YDB_OK != status)
	{
		ydb_zstatus(errbuf, ERRBUF_SIZE);
