...
int main(int argc, char** argv)
{
	...
	printf("### Test error scenarios in ydb_incr_s() of %s Variables ###\n\n", argv[1]); fflush(stdout);
	...
	printf("# Attemping ydb_incr_s() of bad basevar (%% in middle of name): %s\n", badvar1.buf_addr); fflush(stdout);
	status = ydb_incr_s(&badvar1, 0, NULL, NULL, &ret_value);
	if (YDB_OK != status)
	{
	...
	printf("# Attemping ydb_incr_s() of bad basevar (first letter in name is digit): %s\n", badvar2.buf_addr); fflush(stdout);
	status = ydb_incr_s(&badvar2, 0, NULL, NULL, &ret_value);
	if (YDB_OK != status)
	{
		ydb_zstatus(errbuf, ERRBUF_SIZE);
		printf("ydb_incr_s() [%d]: %s\n", __LINE__, errbuf);
	...
	printf("# Attemping ydb_incr_s() of bad basevar (> 31 characters): %s\n", badvar3.buf_addr); fflush(stdout);
	status = ydb_incr_s(&badvar3, 0, NULL, NULL, &ret_value);
	if (YDB_OK != status)
	{
		ydb_zstatus(errbuf, ERRBUF_SIZE);
		printf("ydb_incr_s() [%d]: %s\n", __LINE__, errbuf);
	...
	printf("# Attempting ydb_incr_s() of basevar with 32 subscripts\n"); fflush(stdout);
	for (i = 0; i < 32; i++)
		YDB_LITERAL_TO_BUFFER(SUBSCR32, &subscr32[i]);
	status = ydb_incr_s(&basevar, 32, subscr32, NULL, &ret_value);
	if (YDB_OK != status)
	{
		ydb_zstatus(errbuf, ERRBUF_SIZE);
		printf("ydb_incr_s() [%d]: %s\n", __LINE__, errbuf);
	...
	printf("# Attemtpin ydb_incr_s() of basevar with -1 subscripts\n"); fflush(stdout);
	status = ydb_incr_s(&basevar, -1, NULL, NULL, &ret_value);
	if (YDB_OK != status)
	{
		ydb_zstatus(errbuf, ERRBUF_SIZE);
		printf("ydb_incr_s() [%d]: %s\n", __LINE__, errbuf);
	...
	printf("# Attempting ydb_incr_s() when ret_value->len_alloc is insufficient\n"); fflush(stdout);
	...
	printf("# Attempting ydb_incr_s() with *subsarray->len_alloc < *subsarray->len_used\n"); fflush(stdout);
	subscr1.len_used = subscr1.len_alloc + 1;
	status = ydb_incr_s(&basevar, 1, &subscr1, NULL, &ret_value);
	if (YDB_OK != status)
	{
		ydb_zstatus(errbuf, ERRBUF_SIZE);
		printf("ydb_incr_s() [%d]: %s\n", __LINE__, errbuf);
	...
	subscr1.len_used = 1;
	printf("# Attempting ydb_incr_s() with *increment->len_alloc < *increment->len_used\n"); fflush(stdout);
	incr2.len_used = incr2.len_alloc + 1;
	status = ydb_incr_s(&basevar, 0, NULL, &incr2, &ret_value);
	if (YDB_OK != status)
	{
		ydb_zstatus(errbuf, ERRBUF_SIZE);
		printf("ydb_incr_s() [%d]: %s\n", __LINE__, errbuf);
	...
	printf("# Attempting ydb_incr_s() with *increment->buf_addr set to NULL, and *increment->len_used is non-zero\n"); fflush(stdout);
	incr2.buf_addr = NULL;
	status = ydb_incr_s(&basevar, 0, NULL, &incr2, &ret_value);
	if (YDB_OK != status)
	{
	...
	status = ydb_set_s(&ofvar, 0, NULL, &valueof);
	if (YDB_OK != status)
	{
		ydb_zstatus(errbuf, ERRBUF_SIZE);
		printf("ydb_set_s() [%d]: %s\n", __LINE__, errbuf);
	...
	printf("# Attempting ydb_incr_s() on %s, incrementing by %s, resulting in a numerical overflow\n", ofvar.buf_addr, incrof.buf_addr);
	ret_value.len_used = 4;
	ret_value.len_alloc = 4;
	status = ydb_incr_s(&ofvar, 0, NULL, &incrof, &ret_value);
	if (YDB_OK != status)
	{
		ydb_zstatus(errbuf, ERRBUF_SIZE);
		printf("ydb_incr_s() [%d]: %s\n", __LINE__, errbuf);
	...
	status = ydb_get_s(&ofvar, 0, NULL, &ret_value);
	if (YDB_OK != status)
	{
		ydb_zstatus(errbuf, ERRBUF_SIZE);
		printf("ydb_incr_s() [%d]: %s\n", __LINE__, errbuf);
	...
	printf("# Attempting ydb_incr_s() with *subsarray = NULL\n"); fflush(stdout);
	status = ydb_incr_s(&basevar, 1, NULL, NULL, &ret_value);
	if (YDB_OK != status)
	{
		ydb_zstatus(errbuf, ERRBUF_SIZE);
