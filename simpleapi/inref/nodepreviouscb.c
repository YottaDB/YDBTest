...
/* Routine to help test simpleAPI ydb_node_previous_s(). It is a call-in routine from a MUMPS main process.
 *
 ...
 * (either local or global) that we are running through. Routine runs the subscripted var with ydb_node_previous_s() and dumps the
...
void nodepreviouscb(int parmcnt, ydb_string_t *basevar_in)
{
	 ...
	 * this subscript in the subs_out array as this will become the input value when ydb_node_previous_s()
	 * is called.
	 */
	subs_out_cnt = 1;
