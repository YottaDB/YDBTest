...
int main(int argc, char** argv)
{
	...
	printf("### Test 31-level (max-deep) subscripts can be got using ydb_node_next_s() of %s Variables ###\n", argv[1]); fflush(stdout);
	...
	for (subs = 0; subs < MAX_SUBS; subs++)
	{
		...
		if (subs % 2)
		{
			...
			status = ydb_node_next_s(&basevar, subs, ret_value2, &tmpsubs, ret_value1);
			if (YDB_OK != status)
			{
				if (YDB_ERR_NODEEND != status)
				{
					ydb_zstatus(errbuf, ERRBUF_SIZE);
					printf("ydb_node_next_s() [odd] : subsbuff [%d]: %s\n", subs, errbuf);
					fflush(stdout);
					return YDB_OK;
				} else if (0 != tmpsubs)
				{
					printf("ydb_node_next_s(): *ret_subs_used was not set to 0 but set/left as: %d\n", tmpsubs);
					fflush(stdout);
			...
			} else
			{
				...
				printf("ydb_node_next_s() returned "); fflush(stdout);
		...
		} else
		{
			...
			status = ydb_node_next_s(&basevar, subs, ret_value1, &tmpsubs, ret_value2);
			if (YDB_OK != status)
			{
				if (YDB_ERR_NODEEND != status)
				{
