...
int main()
{
	...
	status = ydb_set_s(&yisv_zmaxtptime, 0, NULL, &value);
	...
	if (2 < (end - begin))
	{
		printf("Timeout test FAILED for ydb_tp_s() : Timeout expected = 1 second. Actual timeout = %d seconds\n",
			(int)(end - begin)); fflush(stdout);
	} else
	{
		printf("Timeout test PASSED for ydb_tp_s() : Timeout expected = 1 second. Actual timeout = %d seconds\n",
			(int)(end - begin)); fflush(stdout);
	}
...
int gvnset()
{
	...
	for (i = 0;  ; i++)
	{
		...
		status = ydb_set_s(&ygbl_tp5, 1, &value, &value);
		if (YDB_OK != status)
			break;
