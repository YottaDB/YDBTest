...
int main()
{
	...
	printf("### Test error scenarios in ydb_get_s() of Local Variables ###\n\n"); fflush(stdout);
	...
	printf("# Test of LVUNDEF error from ydb_get_s()\n"); fflush(stdout);
	status = ydb_get_s(&basevar, 2, subscr32, &ret_value);
	if (YDB_OK != status)
	{
		printf("ydb_get_s() [r]: Return value : Expected = -150373850 (YDB_ERR_LVUNDEF) : Actual = %d\n", status);
		fflush(stdout);
	}
