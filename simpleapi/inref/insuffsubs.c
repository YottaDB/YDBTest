...
int main()
{
	...
	for (i = 0; i < 2; i++)
	{
		...
		/* Set a node for ydb_node_next_s() and ydb_node_previous_s() to return */
		status = ydb_set_s(&basevar, nsubs, subs, NULL);
		YDB_ASSERT(YDB_OK == status);

		/* Test ydb_node_next_s() */
		/* Randomly choose a subscnt that is < MAX_SUBS so we get INSUFFSUBS error */
		outsubscnt = (MAX_SUBS * drand48());
		status = ydb_node_next_s(&basevar, 0, NULL, &outsubscnt, outsubs);
		YDB_ASSERT(YDB_ERR_INSUFFSUBS == status);
		ydb_zstatus(errbuf, ERRBUF_SIZE);
		printf("ydb_node_next_s() : varname = %s : outsubscnt = %d : Returned error : %s\n", basevar.buf_addr, outsubscnt, errbuf); fflush(stdout);
		...
		/* Test ydb_node_previous_s() */
		YDB_LITERAL_TO_BUFFER("100", &maxsub);	/* Choose a value that is greater than current value of subs[0] = "0" */
		/* Randomly choose a subscnt that is < MAX_SUBS so we get INSUFFSUBS error */
		outsubscnt = (MAX_SUBS * drand48());
		status = ydb_node_previous_s(&basevar, 1, &maxsub, &outsubscnt, outsubs);
		YDB_ASSERT(YDB_ERR_INSUFFSUBS == status);
		ydb_zstatus(errbuf, ERRBUF_SIZE);
		printf("ydb_node_previous_s() : varname = %s : outsubscnt = %d : Returned error : %s\n", basevar.buf_addr, outsubscnt, errbuf); fflush(stdout);
		...
		status = ydb_delete_s(&basevar, nsubs, subs, YDB_DEL_NODE);
		YDB_ASSERT(YDB_OK == status);
	}
	return YDB_OK;
}
