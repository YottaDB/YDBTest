...
int main ()
{
	...
	status = ydb_set_s(&basevar, 4, subscr, &value1);
	CHECK_STATUS(status);

	printf("# Calling ydb_get_s()\n");
	status = ydb_get_s(&basevar, 4, subscr, &value2);
	CHECK_STATUS(status);

	printf("# Calling ydb_data_s()\n");
	status = ydb_data_s(&basevar, 4, subscr, &data_value);
	CHECK_STATUS(status);

	printf("# Calling ydb_subscript_next_s()\n");
	status = ydb_subscript_next_s(&basevar, 4, subscr, &value1);
	CHECK_STATUS(status);

	printf("# Calling ydb_subscript_previous_s()\n");
	status = ydb_subscript_previous_s(&basevar, 4, subscr, &value1);
	CHECK_STATUS(status);

	printf("# Calling ydb_node_next_s()\n");
	status = ydb_node_next_s(&basevar, 4, subscr, &dst_used, &value1);
	CHECK_STATUS(status);

	printf("# Calling ydb_node_previous_s()\n");
	status = ydb_node_previous_s(&basevar, 4, subscr, &dst_used, &value1);
	CHECK_STATUS(status);

	printf("# Calling ydb_delete_s()\n");
	status = ydb_delete_s(&basevar, 4, subscr, YDB_DEL_NODE);
	CHECK_STATUS(status);

	printf("# Calling ydb_incr_s()\n");
	status = ydb_incr_s(&basevar, 4, subscr, NULL, &value2);
	CHECK_STATUS(status);
