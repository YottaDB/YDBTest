...
int main()
{
	...
	printf("### Test error scenarios in ydb_subscript_next_s() of Intrinsic Special Variables ###\n\n"); fflush(stdout);
	...
	printf("# Attempting ydb_subscript_next_s() of ISV should issue UNIMPLOP error\n"); fflush(stdout);
	status = ydb_subscript_next_s(&basevar, 0, NULL, &ret_value);
	if (YDB_OK != status)
	{
		ydb_zstatus(errbuf, ERRBUF_SIZE);
		printf("ydb_subscript_next_s() [a]: %s\n", errbuf);
		fflush(stdout);
	}
	return YDB_OK;
}
