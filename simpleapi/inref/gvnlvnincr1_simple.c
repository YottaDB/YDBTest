...
int main(int argc, char** argv)
{
	...
	if (YDB_OK != status)
	{
		if ('^' == basevar.buf_addr[0])
			printf("ydb_get_s() [%d]: Return value : Expected = -150372994 (YDB_ERR_GVUNDEF) : Actual = %d\n", __LINE__, status);
		else
			printf("ydb_get_s() [%d]: Return value : Expected = -150373850 (YDB_ERR_LVUNDEF) : Actual = %d\n", __LINE__, status);
		fflush(stdout);
	}
