...
($gtm_dist/mumps -run srv30^gtm8843 $portno >& server.out &)
$gtm_dist/mumps -run cli30^gtm8843 $portno >& client.out

echo "# This test starts a server-client pair, they connect, then the client"
echo "# triggers the error: extra argument for switching to blocking mode"
echo ---- server ----
cat -n server.out
echo ---- client ----
cat -n client.out
