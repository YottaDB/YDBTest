...
($gtm_dist/mumps -run srv35^gtm8843 $portno >& server.out &)
$gtm_dist/mumps -run cli35^gtm8843 $portno >& client.out

echo "# Trigger invalid second arg error of WRITE /WAIT"
echo ---- server ----
cat -n server.out
echo ---- client ----
cat -n client.out
...
sed -i '/\(SOCKWAITARG\)/d' client.out
