...
($gtm_dist/mumps -run srv28^gtm8843tls $portno >& server.out &)
$gtm_dist/mumps -run cli28^gtm8843tls $portno >& client.out

echo "# This test starts a server-client pair, they connect,"
echo "# the server turns on non-blocking mode, then both sides"
echo "# switch to TLS mode, which shouldn't fail on server side"
echo ---- server ----
cat -n server.out
echo ---- client ----
cat -n client.out
