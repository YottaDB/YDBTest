...
srvRwTest(verb) ;
	...
	.halt
	;
	do checkpoint("server",2,"cli->srv: foo?",0)
	do srvSendResp("bar!")
	do checkpoint("server",3,"srv->cli: bar!",0)
	;
	do checkpoint("server",4,"cli->srv: zeex",0)
	use "server":(socket=conn)
...
cliRwTest(verb) ;
	...
	write /block("off")
	do cliSendReq("foo?")
	do checkpoint("client",2,"cli->srv: foo?",0)
	;
	do checkpoint("client",3,"srv->cli: bar!",0)
	use "client"
	do waitRw("client",verb)
	;
	do cliSendReq("zeex")
	do checkpoint("client",4,"cli->srv: zeex",0)
...
srvStart(trap,mute) ; start server
	...
	;
	do checkpoint("server",1,"server start",0)
 	quit
...
cliConnect(trap,quiet,retryNum) ; connect as client
	...
	do setPort
	do checkpoint("client",1,"server start",0)
	;
	if trap'="" do  quit
	.use $principal
...
checkpoint(actor,id,comment,chkplog) ;
