...
numProc
	...
	ZSYSTEM "$ydb_dist/mupip ftok -jnlpool mumps.repl |& $grep mumps.repl | $tst_awk '{print $6}' >& shm1.out ; ipcs -a | $grep -w `cat ./shm1.out` | $tst_awk '{print $NF}'"
...
ftok
	...
	ZSYSTEM "set repl_ftok_semid = `$gtm_dist/ftok -id=44 $gtm_repl_instance | awk '{print $5}'` ; $gtm_dist/semstat2 `ipcs -s | $grep -w $repl_ftok_semid | awk '{print $2}'` | $grep 'sem 1' | sed 's/sem 1/JNLPOOL ftok : '$gtm_repl_instance' /g' "
