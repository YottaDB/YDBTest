...
trigintrpt
	...
	if ^save'["GTM" write "\$ZJOBEXAM = ",^save,!
	; cleanup globals
...
$echoline
echo 'extract relevant trigger information from the 3 GTM... files created by $ZJOBEXAM'
$echoline
echo ""
set cnt = 1
while ($cnt < 4)
	$tst_awk '/TLEVEL=|ZTLEVEL|ZTOLDVAL|ZTRIGGEROP=|ZTVALUE=/{printf "%s ",$1}' GTM*_$cnt
	echo ""
	@ cnt = $cnt + 1
end
echo ""
...
# which is fired via triggers on both machines.  There will only be 2 GTM... files in the remote directory
# as the second one on the primary is caused by \$ZINTERRUPT which will not occur on the secondary.  That code
# in turn sets ^a to 4 which will then fire the trigger on both systems.  This will result in the 3rd GTM...
