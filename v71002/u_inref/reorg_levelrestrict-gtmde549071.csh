...
$gtm_dist/mumps -run %XCMD 'for i=0:1:$$^%PEEKBYNAME("sgmnt_data.trans_hist.total_blks","DEFAULT")-1 write "dump -block=",$$FUNC^%DH(i)," -header",!' | $gtm_dist/dse | & grep '^Block' | grep -vE "Level 0 |Level -1 |Level 1 " >&! T3indexblocks.out
...
if ($blocks_processed == $index_blocks ) then
	...
	echo "FAIL: MUPIP REORG -MIN_LEVEL=2 specified, but data blocks processed."
endif
