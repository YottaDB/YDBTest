...
;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;
; fiforeadintr.m
; This tests fifo interrupts
; The called entry corresponds to the type of test, e.g. streamnonutf.  It is passed a parameter of this same type to be used
; in globals such as ^readcnt(type) which will record the number of reads done from the fifo.  Since the streaming utf writes are
...
streamnonutf(type)
	...
	do init1(type,.t)
	open t:(fifo:readonly:key=key_" "_iv)
...
fixednonutf(type)
	...
	set a="abcdefghijklmnop0123456789abcdefghijklmnop0123456789abcdefghijklmnop0123456789abcdefghijklmnop0123456789abcdefghijklmnop0123456789"
	do init1(type,.t)
	open t:(fifo:fixed:recordsize=130:readonly:key=key_" "_iv)
	do processStr(type,t,a)
	quit
...
fixedutf8(type)
	...
	do smallutf(.a,1,400)
	do init1(type,.t)
	open t:(fifo:readonly:fixed:recordsize=182:key=key_" "_iv)
...
streamutf16(type)
	...
	do init1(type,.t)
	open t:(fifo:readonly:ICHSET="UTF-16":key=key_" "_iv)
	do processLen(type,t)
	quit
...
fixedutf16(type)
	...
	do smallutf(.a,1,400)
	do init1(type,.t)
	open t:(fifo:readonly:fixed:recordsize=182:ICHSET="UTF-16":key=key_" "_iv)
	do processStr2(type,t,.a)
	quit
...
processLen(type,t)
	; save total char count read for all data read from the fifo and compare with the number sent
	set ^charread(type)=0
	do init2(type,t)
	for i=1:1:^readcnt do
	. set ^readcnt(type)=i
