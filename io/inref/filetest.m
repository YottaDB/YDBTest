filetest ;test various deviceparameters for sequential FILE's
...
itera(openpararametertocontrolbehavior,usepar,closepar,len,expfail) ;
	...
	. ;          1^  2^
	;;;;;;
	if ""'=openpararametertocontrolbehavior do open^io(file,"NEWVERSION:"_openpararametertocontrolbehavior)
	if ""=openpararametertocontrolbehavior do open^io(file,"NEWVERSION")
	do showdev^io(file)
	...
	do close^io(file,closepar)
        if ""'=openpararametertocontrolbehavior do open^io(file,"APPEND:"_openpararametertocontrolbehavior)
        if ""=openpararametertocontrolbehavior do open^io(file,"APPEND")
	do use^io(file,usepar)
	write ";;;;append to file",!
	write $$^longstr(len),!
	write semiline,!
	close file
        if ""'=openpararametertocontrolbehavior do open^io(file,"REWIND:"_openpararametertocontrolbehavior)
        if ""=openpararametertocontrolbehavior do open^io(file,"REWIND")
	if '$DATA(notype) do readfile^filop(file,0)
	if '$DATA(nofinfo)  do
...
error	;
	...
	new $ZTRAP
	set $ZTRAP="write ""TEST-E-TSSERT, should not get an error here"",! halt"
	use $PRINCIPAL
	write "!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!",!
