...
echo '# This used to previously (before YDB@4b499095) fail with a %YDB-F-GTMASSERT2 error'
...
echo ""
echo "------------------------------------------------------------"
echo '# Test EXPR error is issued for incomplete extended reference usages'
echo '# This used to previously (before YDB@4b499095) fail with a %YDB-F-GTMASSERT2 error'
echo "------------------------------------------------------------"
set base = "ydb828extgblincmpl"
cat > $base.m << CAT_EOF
 lock [\$get(x,
 lock [\$order(x,
 lock [\$query(x,
