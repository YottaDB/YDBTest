...
if ($?test_replic) then
	if ($tst_org_host != $tst_remote_host) then
		$rsh $tst_remote_host "cd $tst_remote_dir/$gtm_tst_out/$testname ; source $gtm_tst/com/set_encrypt_env.csh $tst_remote_dir/$gtm_tst_out/$testname $remote_ver $remote_image $tst_src"
	endif
	if ("MULTISITE" == "$test_replic") then
		...
		while ($cntx <= $cnt)
			...
			$rsh $tst_remote_hostx "cd $tst_remote_dirx/$gtm_tst_out/$testname ; source $gtm_tst/com/set_encrypt_env.csh $tst_remote_dirx/$gtm_tst_out/$testname $tst_ver $tst_image $tst_src"
...
if ($?test_gtm_gtcm_one) then
	source $gtm_tst/com/gtcm_command.csh "SEC_SHELL_GTCM ; cd TST_REMOTE_DIR_GTCM/$gtm_tst_out/$testname ; source $gtm_tst/com/set_encrypt_env.csh TST_REMOTE_DIR_GTCM/$gtm_tst_out/$testname $tst_ver $tst_image $tst_src"
endif
