...
##
# Determine if the memory footprint for this machine supports 64GiB journal and receive pools. This awk invocation will
# take care of determining if this test can run restricted 64GiB journal/receive pool tests or not.
...
# Clear subtest_exclude_list file before running subtests which may optionally return a new list of subtests in it
rm -f $tst_general_dir/subtest_exclude_list
...
# Set $subtest_exclude_list to what instream script returned, if it returned any
setenv subtest_exclude_list ""
if ( -e $tst_general_dir/subtest_exclude_list ) then
	setenv subtest_exclude_list "`cat $tst_general_dir/subtest_exclude_list`"
endif

setenv > env.txt   # Record environment used for the test to help with any future analysis

# submit_subtest.csh generates the expected result file outstream.cmp
# If test results contain any lines containing "test start" or "test done",
# add them to the auto-generated outstream.cmp because this is a historical
# and reasonable thing to print at the start and end of tests.
mv $tst_general_dir/outstream.cmp $tst_general_dir/outstream.cmp.original
head -1 $tst_general_dir/outstream.log | grep -iE "tests? (start|begin)" >! $tst_general_dir/outstream.cmp
cat $tst_general_dir/outstream.cmp.original >> $tst_general_dir/outstream.cmp
tail -1 $tst_general_dir/outstream.log | grep -iE "tests? (done|end)" >> $tst_general_dir/outstream.cmp
