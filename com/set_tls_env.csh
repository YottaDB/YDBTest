...
if ("TRUE" == $gtm_test_tls) then
	...
	if (0 != $gtm_test_use_V6_DBs) then
		if (0 != $gtm_test_rhel9_plus) then
			...
			echo "setenv gtm_test_use_V6_DBs $gtm_test_use_V6_DBs"							>>&! settings.csh
		else if (! -e $gtm_root/$gtm_test_v6_dbcreate_rand_ver/pro/plugin/libgtmtls.so) then
			echo '# gtm_test_use_V6_DBs being set to disable, in set_tls_env.csh, due to libgtmtls.so absent for ['$gtm_test_v6_dbcreate_rand_ver']' >>&! settings.csh
			setenv gtm_test_use_V6_DBs 0
			echo "setenv gtm_test_use_V6_DBs $gtm_test_use_V6_DBs"							>>&! settings.csh
		endif
	endif
