...
# With gtm_mupjnl_parallel != 1, it is possible multiple mupip processes generate GTM-F-MEMORY so multiple mupip cores are possible.
...
# Phase 2 : Find value of <limit vmemoryuse> that causes rollback to fail with GTM-F-MEMORY in forward processing phase
...
while (1)
	...
	if ($mid == $midnew) then
		...
		echo "This means the test was not able to find a value of <limit vmemoryuse> that causes rollback to fail with GTM-F-MEMORY in forward processing phase"
...
cp bak/* .  # Restore db/jnl from bak subdirectory before first GTM-F-MEMORY rollback
...
echo ">>> Attempt third (and final) rollback without any GTM-F-MEMORY error. This should work fine"
