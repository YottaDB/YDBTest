...
#################################################################
#
# Test of ydb_incr_st() function for Global and Local variables in the SimpleThreadAPI
#
echo "# Copy all C files that need to be tested"
cp $gtm_tst/$tst/inref/gvnlvnincr*.c .
foreach var_type( gvn lvn )
	echo "# Now run gvnlvnincr*.c (all tests driven by a C routine)"
	foreach file (*"$var_type"*incr*.c)
		...
		if (0 != $status) then
			echo "$var_type""INCR-E-LINKFAIL : Linking $exefile failed. See $exefile.map for details"
			continue
		endif
