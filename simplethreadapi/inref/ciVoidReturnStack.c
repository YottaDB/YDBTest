...
/* Test of ydb_ci_t(YDB_NOTTP, NULL, ) when taking a string as a parameter, and returning void, properly processes the string parameter
 */
...
/* calls ydb_ci_t(YDB_NOTTP, NULL, ) at various nest levels up to 126
 */
int nestHelper(int n){
	int status;
	status = ydb_ci_t(YDB_NOTTP, NULL, "ciStringProc", &toPass, type);
