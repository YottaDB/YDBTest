...
int main()
{
	...
	printf("### Test error scenarios in ydb_delete_st() of Intrinsic Special Variables ###\n\n"); fflush(stdout);
	...
	printf("# Attempting ydb_delete_st() of ISV should issue UNIMPLOP error\n"); fflush(stdout);
	status = ydb_delete_st(YDB_NOTTP, NULL, &basevar, 0, NULL, YDB_DEL_NODE);
	if (YDB_OK != status)
	{
		ydb_zstatus(errbuf, ERRBUF_SIZE);
		printf("ydb_delete_st() [a]: %s\n", errbuf);
		fflush(stdout);
	}
	return YDB_OK;
}
