...
int main()
{
	...
	printf("### Test error scenarios in ydb_get_st() of Global Variables ###\n\n"); fflush(stdout);
	...
	{
		ydb_zstatus(errbuf, ERRBUF_SIZE);
		printf("ydb_get_st() [%d]: %s : ret_value->len_used = %d after call\n", __LINE__, errbuf, ret_value.len_used);
		fflush(stdout);
	}
	...
	printf("# Test of GVUNDEF error from ydb_get_st()\n"); fflush(stdout);
	...
	status = ydb_get_st(YDB_NOTTP, NULL, &isvvar, 1, NULL, &ret_value);
	if (YDB_OK != status)
	{
		ydb_zstatus(errbuf, ERRBUF_SIZE);
		printf("ydb_get_st() [%d]: %s\n", __LINE__, errbuf);
