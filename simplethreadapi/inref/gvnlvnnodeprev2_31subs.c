...
int main(int argc, char** argv)
{
	...
	printf("### Test 31-level (max-deep) subscripts can be got using ydb_node_previous_st() of %s Variables ###\n", argv[1]); fflush(stdout);
	...
	for (subs = MAX_SUBS-1; subs >= 0; subs--)
	{
		...
		if (subs % 2)
		{
			status = ydb_node_previous_st(YDB_NOTTP, NULL, &basevar, subs, ret_value2, &tmpsubs, ret_value1);
			if (YDB_OK != status)
			{
				if (YDB_ERR_NODEEND != status)
				{
					ydb_zstatus(errbuf, ERRBUF_SIZE);
					printf("ydb_node_previous_st() [odd] : subsbuff [%d]: %s\n", subs, errbuf);
					fflush(stdout);
					return YDB_OK;
			...
			} else
			{
				...
				printf("ydb_node_previous_st() returned "); fflush(stdout);
				print_subs(tmpsubs, &basevar, ret_value1);
			}
		} else
		{
			status = ydb_node_previous_st(YDB_NOTTP, NULL, &basevar, subs, ret_value1, &tmpsubs, ret_value2);
			if (YDB_OK != status)
			{
				if (YDB_ERR_NODEEND != status)
				{
					ydb_zstatus(errbuf, ERRBUF_SIZE);
					printf("ydb_node_previous_st() [even] : subsbuff [%d]: %s\n", subs, errbuf);
					fflush(stdout);
					return YDB_OK;
				} else if (tmpsubs != MAX_SUBS)
				{
			...
			} else
			{
				...
				printf("ydb_node_previous_st() returned ");
				fflush(stdout);
