...
int main(int argc, char** argv)
{
	...
	printf("### Test data return in ydb_incr_st() of %s Variables ###\n", argv[1]); fflush(stdout);
	...
	printf("\n# Test ydb_incr_st() on %s, which is not initially set.\n", basevar.buf_addr);
	...
	{
		printf("ydb_incr_st() resulted in an incorrect response\n");
		fflush(stdout);
	} else
	{
		...
		{
			printf("ydb_get_st() returns %s\n", ret_value.buf_addr);
			fflush(stdout);
		}
	}

	printf("\n# Test ydb_incr_st() with *increment set to %s\n", incr3.buf_addr); fflush(stdout);
	status = ydb_incr_st(YDB_NOTTP, NULL, &basevar, 0, NULL, &incr3, &ret_value);
	if (YDB_OK != status)
	{
		ydb_zstatus(errbuf, ERRBUF_SIZE);
		printf("ydb_incr_st[%d]: %s\n", __LINE__, errbuf);
	...
	status = ydb_incr_st(YDB_NOTTP, NULL, &basevar, 0, NULL, &incrm1, &ret_value);
	if (YDB_OK != status)
	{
		ydb_zstatus(errbuf, ERRBUF_SIZE);
		printf("ydb_incr_st[%d]: %s\n", __LINE__, errbuf);
	...
	printf("\n# Test ydb_incr_st() with *increment set to %s in order to decrement to a negative value\n", incrm4.buf_addr); fflush(stdout);
	status = ydb_incr_st(YDB_NOTTP, NULL, &basevar, 0, NULL, &incrm4, &ret_value);
	if (YDB_OK != status)
	{
		ydb_zstatus(errbuf, ERRBUF_SIZE);
		printf("ydb_incr_st[%d]: %s\n", __LINE__, errbuf);
	...
	status = ydb_set_st(YDB_NOTTP, NULL, &basevar, 0, NULL, &valuepure);
	if (YDB_OK != status)
	{
		ydb_zstatus(errbuf, ERRBUF_SIZE);
		printf("ydb_get_st[%d]: %s\n", __LINE__, errbuf);
	...
	printf("# Test ydb_incr_st() when the value of %s is %s, which should be treated as 0, incrementing by 1\n", basevar.buf_addr, valuepure.buf_addr); fflush(stdout);
	status = ydb_incr_st(YDB_NOTTP, NULL, &basevar, 0, NULL, NULL, &ret_value);
	if (YDB_OK != status)
	{
		ydb_zstatus(errbuf, ERRBUF_SIZE);
	...
	printf("\n# Test ydb_incr_st() with *increment set to %s, which should be treated as 0\n", incrpure.buf_addr); fflush(stdout);
	status = ydb_incr_st(YDB_NOTTP, NULL, &basevar, 0, NULL, &incrpure, &ret_value);
	if (YDB_OK != status)
	{
		ydb_zstatus(errbuf, ERRBUF_SIZE);
		printf("ydb_incr_st[%d]: %s\n", __LINE__, errbuf);
	...
	status = ydb_set_st(YDB_NOTTP, NULL, &basevar, 0, NULL, &value6abc);
	if (YDB_OK != status)
	{
		ydb_zstatus(errbuf, ERRBUF_SIZE);
		printf("ydb_get_st[%d]: %s\n", __LINE__, errbuf);
	...
	printf("# Test ydb_incr_st() when the value of %s is %s, and *increment is set to %s\n", basevar.buf_addr, value6abc.buf_addr, incr2abc.buf_addr); fflush(stdout);
	status = ydb_incr_st(YDB_NOTTP, NULL, &basevar, 0, NULL, &incr2abc, &ret_value);
	if (YDB_OK != status)
	{
		ydb_zstatus(errbuf, ERRBUF_SIZE);
		printf("ydb_incr_st[%d]: %s\n", __LINE__, errbuf);
	...
	printf("# Test ydb_incr_st() when the value of %s is %s, and *increment set to %s\n", basevar.buf_addr, value6abc.buf_addr, incrm2abc.buf_addr); fflush(stdout);
	status = ydb_incr_st(YDB_NOTTP, NULL, &basevar, 0, NULL, &incrm2abc, &ret_value);
	if (YDB_OK != status)
	{
		ydb_zstatus(errbuf, ERRBUF_SIZE);
		printf("ydb_incr_st[%d]: %s\n", __LINE__, errbuf);
	...
	status = ydb_set_st(YDB_NOTTP, NULL, &basevar, 0, NULL, &valuem6abc);
	if (YDB_OK != status)
	{
		ydb_zstatus(errbuf, ERRBUF_SIZE);
		printf("ydb_get_st[%d]: %s\n", __LINE__, errbuf);
