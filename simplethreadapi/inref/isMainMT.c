...
int main(){
	...
	if(status == YDB_OK){
		printf("Before ydb_get_st(): ydb_thread_is_main() is correctly returning YDB_OK in main thread\n");
	} else {
		printf("Before ydb_get_st(): ydb_thread_is_main() is not returning YDB_OK in main thread\n");
	}
	...
	status = ydb_thread_is_main();
	if(status == YDB_NOTOK){
		printf("After ydb_get_st(): ydb_thread_is_main() is correctly returning YDB_NOTOK in main thread\n");
	} else {
		printf("After ydb_get_st(): ydb_thread_is_main() is not returning YDB_NOTOK in main thread\n");
	}
