...
{																		\
	...
	{																	\
		if (-YDB_ERR_CALLINAFTERXIT == (uintptr_t)status)										\
			fprintf(file, " --> Invoke %-28s after ydb_exit() : Return value is -YDB_ERR_CALLINAFTERXIT as expected\n", str);	\
		else																\
		{																\
			fprintf(file, " --> Invoke %s after ydb_exit() : Return value : Expected = -YDB_ERR_CALLINAFTERXIT (%d)"		\
	...
	{																	\
		if (0 == status)														\
			fprintf(file, " --> Invoke %-28s after ydb_exit() : Return value is YDB_OK as expected\n", str);			\
		else																\
		{																\
			fprintf(file, " --> Invoke %s after ydb_exit() : Return value : Expected = YDB_OK"					\
...
void *childthread(void *threadparm)
{
	...
	sprintf(fileprint, "CALLINAFTERXIT_%d.txt", tid);
...
/*	To be fixed later: function returns error code that cannot be redirected
	fprintf(fp, " --> Invoke ydb_fork_n_core()            after ydb_exit() : There is no return value since function is void\n"); fflush(stdout);
	ydb_fork_n_core();
*/
	...
	status = ydb_thread_is_main(); CHECK_STATUS("ydb_thread_is_main()", status, fp);
	fprintf(fp, " --> Invoke ydb_timer_cancel()           after ydb_exit() : There is no return value since function is void\n"); fflush(stdout);
