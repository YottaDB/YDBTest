...
int main()
{
	...
	status = ydb_ci_tab_switch_t(YDB_NOTTP, NULL, (uintptr_t)NULL, &prev_handle);
	if (YDB_OK == status)
	...
	status = ydb_ci_tab_open_t(YDB_NOTTP, NULL, "", NULL);
	if (YDB_ERR_PARAMINVALID == status)
	{
		ydb_zstatus(errbuf, ERRBUF_SIZE);
		printf("Got YDB_ERR_PARAMINVALID error as expected : ydb_zstatus() returned : %s\n", errbuf);
	...
	status = ydb_ci_tab_switch_t(YDB_NOTTP, NULL, 0, NULL);
	if (YDB_ERR_PARAMINVALID == status)
	{
		ydb_zstatus(errbuf, ERRBUF_SIZE);
		printf("Got YDB_ERR_PARAMINVALID error as expected : ydb_zstatus() returned : %s\n", errbuf);
	...
	status = ydb_ci_tab_switch_t(YDB_NOTTP, NULL, (uintptr_t)&status, &old_handle[0]);
	if (YDB_ERR_PARAMINVALID == status)
	{
		ydb_zstatus(errbuf, ERRBUF_SIZE);
		printf("Got YDB_ERR_PARAMINVALID error as expected : ydb_zstatus() returned : %s\n", errbuf);
	...
	for (i = 1; i <= NUMTABLES; i++)
	{
		...
		printf("\n# ydb_ci_tab_open_t() : Test that valid call-in table file %s returns YDB_OK\n", citabstr);
		fflush(stdout);
		status = ydb_ci_tab_open_t(YDB_NOTTP, NULL, citabstr, &ret_handle[i - 1]);
		if (YDB_OK == status)
			printf("Got YDB_OK as expected\n");
		else
			printf("Line [%d]: Expected return [%d] but Actual return [%d]\n", __LINE__, YDB_OK, status);
	...
	snprintf(citabstr, sizeof(citabstr), "citable%d.tab", i);
	printf("\n# ydb_ci_tab_open_t() : Test that non-existent call-in table file %s returns YDB_ERR_CITABOPN\n", citabstr);
	fflush(stdout);
	status = ydb_ci_tab_open_t(YDB_NOTTP, NULL, citabstr, &ret_handle[0]);
	...
	{
		snprintf(citabstr, sizeof(citabstr), "citable%d.tab", i);
		printf("\n# ydb_ci_tab_switch_t() : Test switch to valid call-in table file %s returns YDB_OK\n", citabstr);
		fflush(stdout);
		status = ydb_ci_tab_switch_t(YDB_NOTTP, NULL, ret_handle[i - 1], &old_handle[i - 1]);
		if (YDB_OK == status)
			printf("Got YDB_OK as expected\n");
		else
			printf("Line [%d]: Expected return [%d] but Actual return [%d]\n", __LINE__, YDB_OK, status);
	...
	printf("\n# ydb_ci_tab_switch_t() : Test that passing NULL as new_handle returns YDB_OK even if default call-in:"
			"table file [callin.tab] has already been opened\n"); fflush(stdout);
	status = ydb_ci_tab_switch_t(YDB_NOTTP, NULL, prev_handle, &ret_handle[0]);
	if (YDB_OK == status)
		printf("Got YDB_OK as expected\n");
	else
		printf("Line [%d]: Expected return [%d] but Actual return [%d]\n", __LINE__, YDB_OK, status);
	...
	status = ydb_ci_t(YDB_NOTTP, NULL, "citabtest");
	if (YDB_OK == status)
		printf("Got YDB_OK as expected\n");
	else
		printf("Line [%d]: Expected return [%d] but Actual return [%d]\n", __LINE__, YDB_OK, status);
