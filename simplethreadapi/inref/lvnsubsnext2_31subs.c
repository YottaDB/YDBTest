...
int main()
{
	...
	printf("### Test 31-level (max-deep) subscripts can be got using ydb_subscript_next_st() of Local Variables ###\n\n"); fflush(stdout);
	...
	for (subs = 0; subs < MAX_SUBS; subs++)
	{
		...
		status = ydb_subscript_next_st(YDB_NOTTP, NULL, &basevar, subs, subsbuff, &ret_value);
		if (YDB_OK != status)
		{
			ydb_zstatus(errbuf, ERRBUF_SIZE);
			printf("ydb_subscript_next_st() : subsbuff [%d]: %s\n", subs, errbuf);
			fflush(stdout);
			return YDB_OK;
		}
