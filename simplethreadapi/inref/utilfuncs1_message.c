...
int main()
{
	...
	printf("### Test Functionality of ydb_message_t() in the SimpleThreadAPI ###\n"); fflush(stdout);

	printf("\n# Test ydb_message_t() with the errnum for YDB_ERR_PARAMINVALID: %d\n", YDB_ERR_PARAMINVALID); fflush(stdout);

	errnum = YDB_ERR_PARAMINVALID;
	status = ydb_message_t(YDB_NOTTP, NULL, errnum, &err_return);
	if (YDB_OK != status)
	{
	...
	status = ydb_message_t(YDB_NOTTP, NULL, YDB_INT_MAX, &err_return2);
	if (YDB_OK != status)
	{
		...
		} else
		{
			ydb_zstatus(errbuf, ERRBUF_SIZE);
			printf("Line[%d]: %s\n", __LINE__, errbuf);
			printf("and msg_buff remains unchanged\n");
			fflush(stdout);
		}
	} else
	{
	...
	status = ydb_message_t(YDB_NOTTP, NULL, YDB_ERR_PARAMINVALID, &err_return2);
	if (YDB_OK != status)
	{
	...
	status = ydb_message_t(YDB_NOTTP, NULL, YDB_ERR_PARAMINVALID, NULL);
	if (YDB_OK != status)
	{
		ydb_zstatus(errbuf, ERRBUF_SIZE);
		printf("Line[%d]: %s\n", __LINE__, errbuf);
