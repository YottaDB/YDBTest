...
void	glvnZWRITE(uint64_t tptoken, char *startname);
void	glvnZWRITEsubtree(uint64_t tptoken, ydb_buffer_t *basevar, int nsubs, ydb_buffer_t *subscr);
void	glvnPrintNodeIfExists(uint64_t tptoken, ydb_buffer_t *basevar, int nsubs, ydb_buffer_t *subscr, int node_must_exist);

/* Lists ALL local variable nodes in a process in ZWRITE order */
void	lvnZWRITE(uint64_t tptoken)
{
	return glvnZWRITE(tptoken, "%"); /* Go through all local variable nodes */
}

/* Lists ALL global variable nodes in the database in ZWRITE order */
void	gvnZWRITE(uint64_t tptoken)
{
	return glvnZWRITE(tptoken, "^%"); /* Go through all global variable nodes */
}
...
void glvnZWRITE(uint64_t tptoken, char *startname)
{
	...
	if (!rand_chosen)
	{
		...
		srand48(seed);
		/* Randomly choose ydb_subscript_next_st() or ydb_node_next_st() to traverse the Global/Local Variable Tree */
	...
	for (iters=0; ; iters++)
	{
		...
		glvnZWRITEsubtree(tptoken, &basevar, 0, &subscr[0]);
	}
}
...
void glvnZWRITEsubtree(uint64_t tptoken, ydb_buffer_t *basevar, int nsubs, ydb_buffer_t *subscr)
{
	...
	{	/* Use ydb_subscript_next_s to traverse the Global/Local Variable Tree */
		...
		glvnPrintNodeIfExists(tptoken, basevar, nsubs, subscr, NODE_MUST_EXIST_FALSE);
