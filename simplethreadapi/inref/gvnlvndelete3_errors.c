...
int main(int argc, char** argv)
{
	...
	printf("### Test error scenarios in ydb_delete_st() of %s Variables ###\n\n", argv[1]); fflush(stdout);
	...
	if (YDB_OK != status)
	{
		...
		printf("ydb_set_st(YDB_NOTTP, NULL, ) [%d]: %s\n", __LINE__, errbuf);
		fflush(stdout);
	}

	printf("# Test of INVVARNAME error\n"); fflush(stdout);
	printf("# Attemping ydb_delete_st() of bad basevar (%% in middle of name): %s\n", badvar1.buf_addr); fflush(stdout);
	status = ydb_delete_st(YDB_NOTTP, NULL, &badvar1, 0, NULL, YDB_DEL_NODE);
	if (YDB_OK != status)
	{
		ydb_zstatus(errbuf, ERRBUF_SIZE);
		printf("ydb_delete_st() [%d]: %s\n", __LINE__, errbuf);
		fflush(stdout);
	}
	printf("# Attemping ydb_delete_st() of bad basevar (first letter in name is digit): %s\n", badvar2.buf_addr); fflush(stdout);
	status = ydb_delete_st(YDB_NOTTP, NULL, &badvar2, 0, NULL, YDB_DEL_NODE);
	if (YDB_OK != status)
	{
		ydb_zstatus(errbuf, ERRBUF_SIZE);
		printf("ydb_delete_st() [%d]: %s\n", __LINE__, errbuf);
	...
	printf("# Attemping ydb_delete_st() of bad basevar (> 31 characters): %s\n", badvar3.buf_addr); fflush(stdout);
	status = ydb_delete_st(YDB_NOTTP, NULL, &badvar3, 0, NULL, YDB_DEL_NODE);
	if (YDB_OK != status)
	{
		ydb_zstatus(errbuf, ERRBUF_SIZE);
		printf("ydb_delete_st() [%d]: %s\n", __LINE__, errbuf);
	...
	printf("# Attempting ydb_delete_st() of basevar with 32 subscripts\n"); fflush(stdout);
	for (i = 0; i < 32; i++)
		YDB_LITERAL_TO_BUFFER(SUBSCR32, &subscr32[i]);
	status = ydb_delete_st(YDB_NOTTP, NULL, &basevar, 32, subscr32, YDB_DEL_NODE);
	if (YDB_OK != status)
	{
		ydb_zstatus(errbuf, ERRBUF_SIZE);
		printf("ydb_delete_st() [%d]: %s\n", __LINE__, errbuf);
	...
	printf("# Attempting ydb_delete_st() of basevar with -1 subscripts\n"); fflush(stdout);
	status = ydb_delete_st(YDB_NOTTP, NULL, &basevar, -1, NULL, YDB_DEL_NODE);
	if (YDB_OK != status)
	{
		ydb_zstatus(errbuf, ERRBUF_SIZE);
		printf("ydb_delete_st() [%d]: %s\n", __LINE__, errbuf);
	...
	printf("# Attempting ydb_delete_st() with deltype != YDB_DEL_NODE/YDB_DEL_TREE\n"); fflush(stdout);
	status = ydb_delete_st(YDB_NOTTP, NULL, &basevar, 0, NULL, YDB_DEL_NODE+100);
	if (YDB_OK != status)
	{
		ydb_zstatus(errbuf, ERRBUF_SIZE);
		printf("ydb_delete_st() [%d]: %s\n", __LINE__, errbuf);
	...
	printf("# Attempting ydb_delete_st() with *subsarray->len_alloc < *subsarray->len_used\n"); fflush(stdout);
	subscr1.len_used = subscr1.len_alloc + 1;
	status = ydb_delete_st(YDB_NOTTP, NULL, &basevar, 1, &subscr1, YDB_DEL_NODE);
	if (YDB_OK != status)
	{
		ydb_zstatus(errbuf, ERRBUF_SIZE);
		printf("ydb_delete_st() [%d]: %s\n", __LINE__, errbuf);
	...
	subscr1.len_used = 1;
	printf("# Attempting ydb_delete_st() with *subsarray->buf_addr set to NULL, and *subsarray->len_used is non-zero\n"); fflush(stdout);
	...
	printf("# Attempting ydb_delete_st() with *subsarray = NULL\n"); fflush(stdout);
	status = ydb_delete_st(YDB_NOTTP, NULL, &basevar, 1, NULL, YDB_DEL_NODE);
	if (YDB_OK != status)
	{
		ydb_zstatus(errbuf, ERRBUF_SIZE);
