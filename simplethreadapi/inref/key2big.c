...
int main ()
{
	...
	status = ydb_get_st(YDB_NOTTP, NULL, &basevar, 0, NULL, &value2);
	CHECK_STATUS(status);

	printf("# Calling ydb_data_st()\n");
	status = ydb_data_st(YDB_NOTTP, NULL, &basevar, 0, NULL, &data_value);
	CHECK_STATUS(status);

	printf("# Calling ydb_subscript_next_st()\n");
	status = ydb_subscript_next_st(YDB_NOTTP, NULL, &basevar, 0, NULL, &value1);
	CHECK_STATUS(status);

	printf("# Calling ydb_subscript_previous_st()\n");
	status = ydb_subscript_previous_st(YDB_NOTTP, NULL, &basevar, 0, NULL, &value1);
	CHECK_STATUS(status);

	printf("# Calling ydb_node_next_st()\n");
	status = ydb_node_next_st(YDB_NOTTP, NULL, &basevar, 0, NULL, &dst_used, &value1);
	CHECK_STATUS(status);

	printf("# Calling ydb_node_previous_st()\n");
	status = ydb_node_previous_st(YDB_NOTTP, NULL, &basevar, 0, NULL, &dst_used, &value1);
	CHECK_STATUS(status);
	...
	status = ydb_incr_st(YDB_NOTTP, NULL, &basevar, 0, NULL, NULL, &value2);
	CHECK_STATUS(status);
