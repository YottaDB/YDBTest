...
int main(int argc, char** argv)
{
	...
	printf("### Test 31-level (max-deep) subscripts can be used while using ydb_incr_st() of %s Variables ###\n", argv[1]); fflush(stdout);
	...
	for (subs = 0; subs < MAX_SUBS; subs++)
	{
		...
		status = ydb_incr_st(YDB_NOTTP, NULL, &basevar, subs, subsbuff, &incr, &ret_value);
		if (YDB_OK != status)
		{
			ydb_zstatus(errbuf, ERRBUF_SIZE);
			printf("ydb_set_st() : subsbuff [%d]: %s\n", subs, errbuf);
