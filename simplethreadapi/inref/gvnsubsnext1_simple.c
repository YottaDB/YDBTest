...
int main()
{
	...
	char		errbuf[ERRBUF_SIZE], retvaluebuff1[64], retvaluebuff2[64], rettestbuff[64];
	...
	status = ydb_subscript_next_st(YDB_NOTTP, NULL, &basevar, 0, NULL, &ret_value1);
	if (YDB_OK != status)
	{
		ydb_zstatus(errbuf, ERRBUF_SIZE);
	...
	status = ydb_subscript_next_st(YDB_NOTTP, NULL, &nextvar, 0, NULL, &ret_value1);
	...
	status = ydb_subscript_next_st(YDB_NOTTP, NULL, &basevar, 1, subscr, &ret_value1);
	if (YDB_OK != status)
	{
		ydb_zstatus(errbuf, ERRBUF_SIZE);
	...
	status = ydb_subscript_next_st(YDB_NOTTP, NULL, &basevar, 2, subscr, &ret_value1);
	if (YDB_OK != status)
	{
		ydb_zstatus(errbuf, ERRBUF_SIZE);
