...
/* Routine to help test SimpleThreadAPI ydb_node_next_st(). It is a call-in routine from a MUMPS main process.
 *
 ...
 * (either local or global) that we are running through. Routine runs the subscripted var with ydb_node_next_st() and dumps the
...
void nodenextcb(int parmcnt, ydb_string_t *basevar_in)
{
	...
	{	/* Once through for each node - subs_in_cnt gets initialized to subs_out_cnt except for
		...
		status = ydb_node_next_st(YDB_NOTTP, NULL, &basevar, subs_in_cnt, subs_in, &subs_out_cnt, subs_out);
