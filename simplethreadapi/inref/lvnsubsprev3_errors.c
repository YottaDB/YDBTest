...
int main()
{
	...
	status = ydb_subscript_previous_st(YDB_NOTTP, &badbasevar, 0, NULL, &ret_value);
	if (YDB_OK != status)
	{
	...
	status = ydb_subscript_previous_st(YDB_NOTTP, &basevar, 32, subscr32, &ret_value);
	if (YDB_OK != status)
	{
		ydb_zstatus(errbuf, ERRBUF_SIZE);
		printf("ydb_subscript_previous_st() [%d]: %s\n", __LINE__, errbuf);
	...
	status = ydb_subscript_previous_st(YDB_NOTTP, &basevar, -1, NULL, &ret_value);
	if (YDB_OK != status)
	{
		ydb_zstatus(errbuf, ERRBUF_SIZE);
		printf("ydb_subscript_previous_st() [%d]: %s\n", __LINE__, errbuf);
	...
	status = ydb_subscript_previous_st(YDB_NOTTP, &basevar, 0, NULL, NULL);
	if (YDB_OK != status)
	{
		ydb_zstatus(errbuf, ERRBUF_SIZE);
		printf("ydb_subscript_previous_st() [%d]: %s\n", __LINE__, errbuf);
	...
	status = ydb_subscript_previous_st(YDB_NOTTP, &nextvar, 0, NULL, NULL);
	if (YDB_OK != status)
	{
		ydb_zstatus(errbuf, ERRBUF_SIZE);
		printf("ydb_subscript_previous_st() [%d]: %s\n", __LINE__, errbuf);
	...
	ret_value.buf_addr = NULL;
	ret_value.len_used = getpid() % 2;	/* this value does not matter to the final error, hence the randomization */
	status = ydb_subscript_previous_st(YDB_NOTTP, &nextvar, 0, NULL, &ret_value);
	if (YDB_OK != status)
	{
		ydb_zstatus(errbuf, ERRBUF_SIZE);
		printf("ydb_subscript_previous_st() [%d]: %s\n", __LINE__, errbuf);
