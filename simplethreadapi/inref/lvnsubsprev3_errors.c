...
int main()
{
	...
	printf("### Test error scenarios in ydb_subscript_previous_st() of Local Variables ###\n\n"); fflush(stdout);
	...
	printf("Attempting ydb_subscript_previous_st() of bad basevar (%% in middle of name) %s\n", BADBASEVAR1);
	fflush(stdout);
	YDB_LITERAL_TO_BUFFER(BADBASEVAR1, &badbasevar);
	status = ydb_subscript_previous_st(YDB_NOTTP, NULL, &badbasevar, 0, NULL, &ret_value);
	if (YDB_OK != status)
	{
	...
	printf("Attempting ydb_subscript_previous_st() of bad basevar (> 31 characters) %s\n", BADBASEVAR2);
	...
	printf("Attempting ydb_subscript_previous_st() of bad basevar (first letter in name is digit) %s\n", BADBASEVAR3);
	...
	printf("Attempting ydb_subscript_previous_st() of basevar with 32 subscripts\n");
	for (i = 0; i < 32; i++)
		YDB_LITERAL_TO_BUFFER(SUBSCR32, &subscr32[i]);
	status = ydb_subscript_previous_st(YDB_NOTTP, NULL, &basevar, 32, subscr32, &ret_value);
	if (YDB_OK != status)
	{
		ydb_zstatus(errbuf, ERRBUF_SIZE);
		printf("ydb_subscript_previous_st() [%d]: %s\n", __LINE__, errbuf);
	...
	printf("Attempting ydb_subscript_previous_st() of basevar with -1 subscripts\n");
	status = ydb_subscript_previous_st(YDB_NOTTP, NULL, &basevar, -1, NULL, &ret_value);
	if (YDB_OK != status)
	{
		ydb_zstatus(errbuf, ERRBUF_SIZE);
		printf("ydb_subscript_previous_st() [%d]: %s\n", __LINE__, errbuf);
	...
	printf("Attempting ydb_subscript_previous_st() with ret_value->buf_addr=NULL when return value is a NULL string : Expect NO PARAMINVALID error\n");
	...
	printf("Attempting ydb_subscript_previous_st() with NULL ret_value when return value is a NULL string : Expect PARAMINVALID error\n");
	status = ydb_subscript_previous_st(YDB_NOTTP, NULL, &basevar, 0, NULL, NULL);
	if (YDB_OK != status)
	{
		ydb_zstatus(errbuf, ERRBUF_SIZE);
		printf("ydb_subscript_previous_st() [%d]: %s\n", __LINE__, errbuf);
	...
	printf("Attempting ydb_subscript_previous_st() with NULL ret_value when return value is a non-NULL string : Expect PARAMINVALID error\n");
	...
	status = ydb_subscript_previous_st(YDB_NOTTP, NULL, &nextvar, 0, NULL, NULL);
	if (YDB_OK != status)
	{
		ydb_zstatus(errbuf, ERRBUF_SIZE);
		printf("ydb_subscript_previous_st() [%d]: %s\n", __LINE__, errbuf);
	...
	printf("Attempting ydb_subscript_previous_st() with ret_value->buf_addr=NULL when return value is a non-NULL string : Expect PARAMINVALID error\n");
	ret_value.buf_addr = NULL;
	ret_value.len_used = getpid() % 2;	/* this value does not matter to the final error, hence the randomization */
	status = ydb_subscript_previous_st(YDB_NOTTP, NULL, &nextvar, 0, NULL, &ret_value);
	if (YDB_OK != status)
	{
		ydb_zstatus(errbuf, ERRBUF_SIZE);
		printf("ydb_subscript_previous_st() [%d]: %s\n", __LINE__, errbuf);
	...
	printf("Attempting ydb_subscript_previous_st() with ret_value->len_alloc=0 : Expect INVSTRLEN error\n");
	...
	{
		ydb_zstatus(errbuf, ERRBUF_SIZE);
		printf("ydb_subscript_previous_st() [%d]: %s : ret_value->len_used = %d after call\n", __LINE__, errbuf, ret_value.len_used);
		fflush(stdout);
	}
	ret_value.len_alloc = ret_value.len_used;
	printf("Attempting ydb_subscript_previous_st() with ret_value->len_alloc set to value returned in ret_value->len_used after INVSTRLEN error. Expect NO error\n");
	status = ydb_subscript_previous_st(YDB_NOTTP, NULL, &nextvar, 0, NULL, &ret_value);
	if (YDB_OK != status)
	{
		ydb_zstatus(errbuf, ERRBUF_SIZE);
	...
	printf("Attempting ydb_subscript_previous_st() of basevar with NULL subsarray parameter. Expect SUBSARRAYNULL error\n");
	status = ydb_subscript_previous_st(YDB_NOTTP, NULL, &basevar, 1, NULL, &ret_value);
	if (YDB_OK != status)
	{
		ydb_zstatus(errbuf, ERRBUF_SIZE);
	...
	{
		printf("Attempting ydb_subscript_previous_st() with subsarray[%d].len_alloc=0 and subsarray[%d].len_used=2 : Expect PARAMINVALID error\n", i, i);
		save_subscr32 = subscr32[i];
		subscr32[i].len_alloc = 0;
		subscr32[i].len_used = 2;
		status = ydb_subscript_previous_st(YDB_NOTTP, NULL, &basevar, 5, subscr32, &ret_value);
		if (YDB_OK != status)
		{
			ydb_zstatus(errbuf, ERRBUF_SIZE);
			printf("ydb_subscript_previous_st() [%d]: %s\n", __LINE__, errbuf);
		...
		printf("Attempting ydb_subscript_previous_st() with subsarray[%d].len_alloc=0 and subsarray[%d].len_used=0 : Expect NO PARAMINVALID error\n", i, i);
		subscr32[i].len_alloc = 0;
		...
		printf("Attempting ydb_subscript_previous_st() with subscr32[%d].buf_addr=NULL and subscr32[%d].len_used=0 : Expect NO PARAMINVALID error\n", i, i);
		subscr32[i].buf_addr = NULL;
		subscr32[i].len_used = 0;
		status = ydb_set_st(YDB_NOTTP, NULL, &basevar, 5, subscr32, &ret_value);
		if (YDB_OK != status)
	...
	printf("# Test that LVUNDEF error is NOT issued by ydb_subscript_previous_st()\n"); fflush(stdout);
	status = ydb_subscript_previous_st(YDB_NOTTP, NULL, &basevar, 2, subscr32, &ret_value);
	if (YDB_ERR_NODEEND == status)
