	; External filter, simplistic echo back filter which replaces ^x(1) with null record
	; Modified copy of ext_filter/inref/manynulls.m
	set $ztrap="goto err"
	set TSTART="08"
	set TCOMMIT="09"
        set NULL="00"
	...
	... if $zeof halt
        .. set node=$p(extrRec,"\",11)
	.. set node=$p(node,"=",1)
	.. if node="^x(1)" set filtrOut=NULLREC_EOL
