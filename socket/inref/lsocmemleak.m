...
lsocketmemleak
	;;; lsocmemleak.m
	; Test LOCAL socket open failure for memory leak
	;   "server" just keeps the port busy so the "client" errors when
	;   trying to open the same path
	;
	; ------------------------
	; configure the test
	; ------------------------
...
socketexistnd
	...
	. o tcpdev:(ZLISTEN=path_":LOCAL":delimiter="":attach="LISTENER":new):1:"SOCKET"
	. e  w !,"FAILED to open the socket device at "_$zpos,! s done=1 q
	. o tcpdev:(ZLISTEN=path_":LOCAL":delimiter="":attach="LISTENER":exception="g incrtrap^incrtrap":new):1:"SOCKET"
	. c tcpdev:(NODEST)
...
socketexistd
	...
	. s:repno=skpcnt startused=$zus
	. s:repno=repcnt endused=$zus
	. o tcpdev:(ZLISTEN=path_":LOCAL":delimiter="abcdefg":attach="LISTENER":new):1:"SOCKET"
	. e  w "FAILED to open the socket device at "_$zpos,! s done=1 q
	. o tcpdev:(ZLISTEN=path_":LOCAL":delimiter="abcdefg":attach="LISTENER":exception="g incrtrap^incrtrap":new):1:"SOCKET"
	. c tcpdev:(NODEST)
	i startused<endused s cnt=repcnt-skpcnt,leak=endused-startused w "  FAILED: Memory leak: In "_cnt_" iterations, leaked "_leak_" bytes",!
	e  w "  PASSED",!
	; ------------------------
...
delimwidthd
	...
	. s:repno=skpcnt startused=$zus
	. s:repno=repcnt endused=$zus
	. o tcpdev:(ZLISTEN=path_":LOCAL":delimiter=delim:attach="LISTENER":new):1:"SOCKET"
	. c tcpdev:(NODEST)
	u 0
	i startused<endused s cnt=repcnt-skpcnt,leak=endused-startused w "  FAILED: Memory leak: In "_cnt_" iterations, leaked "_leak_" bytes",!
	e  w "  PASSED",!
...
socmaxnd
	...
	. . o tcpdev:(ZLISTEN=path_"."_sock_":LOCAL":delimiter="":attach="listen"_sock:exception="g incrtrap^incrtrap":new):1:"SOCKET"
...
socmaxd
	...
	. s:repno=repcnt endused=$zus
	. f sock=1:1:1000 d  i $p($zs,",",1)=150381146 s $zs="" q
	. . o tcpdev:(ZLISTEN=path_"."_sock_":LOCAL":delimiter="abcdefg":attach="listen"_sock:exception="g incrtrap^incrtrap":new):1:"SOCKET"
	. . ;Empty line where incrtrap returns
	. f sock=1:1:sock-1 c tcpdev:(NODEST:socket="listen"_sock)
	. c tcpdev
	i startused<endused s cnt=repcnt-skpcnt,leak=endused-startused w "  FAILED: Memory leak: In "_cnt_" iterations, leaked "_leak_" bytes",! zwrite startused,endused
...
connectnd
	...
	. o tcpdev:(connect=socket:delimiter="":attach="connect":exception="g incrtrap^incrtrap"):1:"SOCKET"
...
connectd
	...
	. s:repno=skpcnt startused=$zus
	. s:repno>skpcnt endused=$zus
	. o tcpdev:(connect=socket:delimiter="abcdefg":attach="connect":exception="g incrtrap^incrtrap"):1:"SOCKET"
	. i  c tcpdev:(NODEST) w "FAILED: socket open succeeded at "_$zpos s done=1 q
	i startused<endused s cnt=repno-skpcnt,leak=endused-startused w "  FAILED: Memory leak: In "_cnt_" iterations, leaked "_leak_" bytes",!
	e  w "  PASSED",!
	;
	...
	; ------------------------
	quit	; the rest of the tests, including lsocmlkclnt fail with
		; LOCAL sockets since ZLISTEN deletes an existing socket.
		; Even for TCP sockets the rest of tests are unclear.
	;
	; ------------------------
