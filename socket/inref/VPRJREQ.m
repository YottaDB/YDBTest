...
START(TCPPORT,DEBUG) ; set up listening for connections
 ...
 U TCPIO
 ;
 I %WOS="GT.M" I +$DEVICE U 0 W !,"error opening server: "_$DEVICE Q	; SMW
...
GTMLNX  ;From Linux xinetd script; $P is the main stream
 ...
 X "U $P:(nowrap:nodelimiter:ioerror=""t"":exception=""ETSOCK"")"	; SMW
...
TLS ; Turn on TLS?
 ; Need to do this for Cache and GT.M
  U %WTCP:ioerror="t"						; SMW
  W /TLS("server",1,"server")					; SMW
  N D,K,T
  S D=$DEVICE,K=$KEY,T=$TEST
  I +D G ETSOCK^VPRJREQ						; SMW
  ; U 0
  ; W !
  ; W "$DEVICE: "_D_$c(10),!
  ; W "$KEY: "_K_$c(10),!
...
LOGERR ; log error information
 ...
 Q
LOGINFO(X,WHAT) ; log stuff					; SMW
 N DT,ID,LN							; SMW
 S DT=HTTPLOG("DT"),ID=HTTPLOG("ID")				; SMW
 S ^VPRHTTP("log",DT,$J,ID,WHAT)=X				; SMW
 Q								; SMW
 ;
