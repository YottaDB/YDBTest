...
#################################################################
#
# Routine to cause a SETZDIR error to be raised which usually logs to the syslog unless the userid's group is listed
...
# Drive YDB with a statement to try to set a node
$ydb_dist/mumps -run ^%XCMD 'write "DrivePID: ",$job,! set $zdir="/blat"'
exit $status
