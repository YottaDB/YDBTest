...
# Extract the PIDs of the jobs spawned by [replwason.m] and check only for syslog messages from these PIDs to
# prevent false positives from messages generated by processes from concurrently running tests.
$gtm_dist/mumps -run %XCMD 'zwrite ^%jobwait' >&! INST1-jobs.out
set job_pids = `grep -E "jobwait\([1-5]\)" INST1-jobs.out | cut -f 2 -d '='`
grep -Ew ".*%YDB-E-JNLCLOSED.*%YDB-I-REPLSTATE.*" test_syslog1.txt | grep -E "`echo $job_pids | sed 's/ /\|/g'`"
