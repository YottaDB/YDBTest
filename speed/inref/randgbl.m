...
SETOP;  Sets 
	...
	.  SET ndx=1
	.  SET root=root(rep+jobno#totroot+1)
	.  FOR i=1:1:prime-1 D
...
READOP; Reads
	...
	.  .  SET RES=^afill(jobno,ndx)
        ...
        .  .  SET RES=^efill(rep,jobno,ndx)
        .  .  SET RES=^ffill(cndx,ndx,3.14)
        .  .  SET RES=^gfill(jobno,ndx)
        .  .  SET RES=^hfill(cndx,jobno)
        SET t2=$ZGETJPI(0,"CPUTIM")
        SET et2=$H
	do RESULT^randgbl(^opname(2),t1,t2,et1,et2)
	q
GETOP; $GET
	SET ^opname(3)="GETOP"
	SET et1=$H
	...
	.  .  SET RES=$GET(^afill(jobno,ndx))
        ...
        .  .  SET RES=$GET(^efill(rep,jobno,ndx))
        .  .  SET RES=$GET(^ffill(cndx,ndx,3.14))
        .  .  SET RES=$GET(^gfill(jobno,ndx))
        .  .  SET RES=$GET(^hfill(cndx,jobno))
        SET t2=$ZGETJPI(0,"CPUTIM")
        SET et2=$H
	do RESULT^randgbl(^opname(3),t1,t2,et1,et2)
	q
DATAOP; $DATA
	SET ^opname(4)="DATAOP"
	SET et1=$H
	...
	.  .  SET RES=$DATA(^afill(jobno,ndx))
        ...
        .  .  SET RES=$DATA(^efill(rep,jobno,ndx))
        .  .  SET RES=$DATA(^ffill(cndx,ndx,3.14))
        .  .  SET RES=$DATA(^gfill(jobno,ndx))
        .  .  SET RES=$DATA(^hfill(cndx,jobno))
        SET t2=$ZGETJPI(0,"CPUTIM")
        SET et2=$H
	do RESULT^randgbl(^opname(4),t1,t2,et1,et2)
	q
ORDEROP; $ORDER
	...
	.  .  SET RES=$ORDER(^afill(jobno,ndx))
        ...
        .  .  SET RES=$ORDER(^efill(rep,jobno,ndx))
        .  .  SET RES=$ORDER(^ffill(cndx,ndx,3.14))
        .  .  SET RES=$ORDER(^gfill(jobno,ndx))
        .  .  SET RES=$ORDER(^hfill(cndx,jobno))
        SET t2=$ZGETJPI(0,"CPUTIM")
        SET et2=$H
	do RESULT^randgbl(^opname(5),t1,t2,et1,et2)
	q
ZPREVOP; $ORDER(x,-1)
	...
	.  .  SET RES=$ORDER(^afill(jobno,ndx),-1)
        ...
        .  .  SET RES=$ORDER(^efill(rep,jobno,ndx),-1)
        .  .  SET RES=$ORDER(^ffill(cndx,ndx,3.14),-1)
        .  .  SET RES=$ORDER(^gfill(jobno,ndx),-1)
        .  .  SET RES=$ORDER(^hfill(cndx,jobno),-1)
        SET t2=$ZGETJPI(0,"CPUTIM")
        SET et2=$H
	do RESULT^randgbl(^opname(6),t1,t2,et1,et2)
	q
QUERYOP; $QUERY
	...
	.  .  SET RES=$QUERY(^afill(jobno,ndx))
        ...
        .  .  SET RES=$QUERY(^efill(rep,jobno,ndx))
        .  .  SET RES=$QUERY(^ffill(cndx,ndx,3.14))
        .  .  SET RES=$QUERY(^gfill(jobno,ndx))
        .  .  SET RES=$QUERY(^hfill(cndx,jobno))
        SET t2=$ZGETJPI(0,"CPUTIM")
        SET et2=$H
	do RESULT^randgbl(^opname(7),t1,t2,et1,et2)
	q
KILLOP; KILL
	SET ^opname(8)="KILLOP"
	SET et1=$H
	...
	.  .  KILL ^afill(jobno,ndx)
        ...
        .  .  KILL ^efill(rep,jobno,ndx)
        .  .  KILL ^ffill(cndx,ndx,3.14)
        .  .  KILL ^gfill(jobno,ndx)
        .  .  KILL ^hfill(cndx,jobno)
        SET t2=$ZGETJPI(0,"CPUTIM")
        SET et2=$H
	do RESULT^randgbl(^opname(8),t1,t2,et1,et2)
	q
