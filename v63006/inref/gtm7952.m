...
gtm7952
	...
	. write:test=1 "(expect success)",!
	. write:test=2 !,"# Second test using external call sigdisable1() which has SIGSAFE defined on the external call "
	. write:test=2 "(expect HANG command to hang until killed on GT.M, and to pass on YottaDB)",!
	. ;
	. set ^test=test
...
; Note: expects ^i to be set to an incremented value.
;
hangtest
	;
...
hangkill(hangpid,waitsec)
	...
	;
	; Wait a few more seconds for the hang process to wakeup and shutdown
	;
	for i=1:1:5 quit:1=^done  hang 1
	write "hangkill: Sleeping complete - checking hang process now",!
	;
	; Now see if the hang process has completed its HANGs or not
	;
	if ^done write "hangkill: Found hang in complete status so exiting",!
	else  do
	. write "hangkill: Found hang (pid ",hangpid,") in a NOT complete status so sending it a SIGTERM interrupt",!
	. set x=$zsigproc(hangpid,15) 	; Send SIGTERM
	. write:0'=x "hangkill: $ZSIGPROC() to kill hang pid failed with rc ",x,!
