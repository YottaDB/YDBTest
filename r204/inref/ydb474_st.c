...
int main(void)
{
	...
	YDB_ASSERT(done);
	status = ydb_decode_st(YDB_NOTTP, NULL, &variable, num_subscripts, subscripts, YDB_MAX_SUBS, format, json_object);
	if (YDB_OK != status)
	{
		ydb_zstatus(errbuf, YDB_MAX_ERRORMSG);
	...
	YDB_ASSERT(done);
	status = ydb_decode_st(YDB_NOTTP, NULL, &variable, num_subscripts, subscripts, YDB_MAX_SUBS, format, json_array);
	if (YDB_OK != status)
	{
		ydb_zstatus(errbuf, YDB_MAX_ERRORMSG);
		printf("Error: %s:%d: ydb_decode_st() : %s\n", __FILE__, __LINE__, errbuf);
	...
	YDB_ASSERT(done);
	status = ydb_decode_st(YDB_NOTTP, NULL, &variable, num_subscripts, subscripts, YDB_MAX_SUBS, format, json_object_max_depth);
	if (YDB_OK != status)
	{
		ydb_zstatus(errbuf, YDB_MAX_ERRORMSG);
		printf("Error: %s:%d: ydb_decode_st() : %s\n", __FILE__, __LINE__, errbuf);
	...
	YDB_ASSERT(done);
	status = ydb_decode_st(YDB_NOTTP, NULL, &variable, num_subscripts, subscripts, YDB_MAX_SUBS, format, json_true_false_null);
	if (YDB_OK != status)
	{
		ydb_zstatus(errbuf, YDB_MAX_ERRORMSG);
		printf("Error: %s:%d: ydb_decode_st() : %s\n", __FILE__, __LINE__, errbuf);
	...
	YDB_ASSERT(done);
	status = ydb_decode_st(YDB_NOTTP, NULL, &variable, num_subscripts, subscripts,
			YDB_MAX_SUBS, format, json_too_many_subscripts);
	if (YDB_OK != status)
	{
		ydb_zstatus(errbuf, YDB_MAX_ERRORMSG);
		printf("%s:%d: ydb_decode_st() : %s\n", __FILE__, __LINE__, errbuf);
		fflush(stdout);
	...
	YDB_ASSERT(done);
	status = ydb_decode_st(YDB_NOTTP, NULL, &variable, num_subscripts, subscripts,
			YDB_MAX_SUBS, format, json_data_and_children);
	if (YDB_OK != status)
	{
		ydb_zstatus(errbuf, YDB_MAX_ERRORMSG);
		printf("Error: %s:%d: ydb_decode_st() : %s\n", __FILE__, __LINE__, errbuf);
	...
	YDB_ASSERT(done);
	status = ydb_decode_st(YDB_NOTTP, NULL, &variable, num_subscripts, subscripts, YDB_MAX_SUBS, format, json_invalid);
	if (YDB_OK != status)
	{
		ydb_zstatus(errbuf, YDB_MAX_ERRORMSG);
		printf("%s:%d: ydb_decode_st() : %s\n", __FILE__, __LINE__, errbuf);
	...
	YDB_ASSERT(done);
	status = ydb_decode_st(YDB_NOTTP, NULL, &variable, num_subscripts, subscripts, YDB_MAX_SUBS, format, json_large_buffer);
	if (YDB_OK != status)
	{
		ydb_zstatus(errbuf, YDB_MAX_ERRORMSG);
		printf("Error: %s:%d: ydb_decode_st() : %s\n", __FILE__, __LINE__, errbuf);
	...
	YDB_ASSERT(done);
	status = ydb_decode_st(YDB_NOTTP, NULL, &variable, num_subscripts, subscripts,
			YDB_MAX_SUBS, format, json_long_keyvalue_buffer);
	if (YDB_OK != status)
	{
		ydb_zstatus(errbuf, YDB_MAX_ERRORMSG);
		printf("Error: %s:%d: ydb_decode_st() : %s\n", __FILE__, __LINE__, errbuf);
	...
	YDB_COPY_LITERAL_TO_BUFFER("^jsonObject", &variable, done);
	YDB_ASSERT(done);
	status = ydb_encode_st(YDB_NOTTP, NULL, &variable, num_subscripts, subscripts, format, &data);
	if (YDB_OK != status)
	{
		ydb_zstatus(errbuf, YDB_MAX_ERRORMSG);
