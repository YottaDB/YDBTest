...
;								;
;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;
; See u_inref/rqtest08.csh for purpose of test. Also see reference file outref/rqtest08.txt for a better picture.
;
rqtest08 ;
	set ^x=0
	set ^x(1,0)=1
	...
	set ^x(1,2,2,4)=5
	write " --> Global variable tree on which $query operations are performed",!
	zwrite ^x
	set prev="^x"
	write !," ---> $query(gvn,1) and $query(gvn,-1) on existing gv tree nodes",!
	...
	write !," ---> $query(gvn,1) on non-existing gv tree nodes",!
	for j=1:1:i  do
